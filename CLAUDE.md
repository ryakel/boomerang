...
- Watch/app icon problems are never diagnosable from the build log — an empty icon set, a stale asset catalog, and a missing `CFBundleIconName` all end in BUILD SUCCEEDED with the same placeholder. Run `npm run ios:watch-doctor` and read the bundle. Targets with `GENERATE_INFOPLIST_FILE = NO` must carry `CFBundleIconName` themselves.
