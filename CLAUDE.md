...
**Management UI:** `GrowthAreasModal` (`src/components/GrowthAreasModal.jsx`) — simplest CRUD surface in the app, on par with Labels. Add/edit expose Morning/Evening/Persistent checkboxes + a day-scope select (Any day/Weekdays/Weekends). Entry points: System menu (legacy theme), More → Growth areas (Kept mobile), sidebar nav (Kept desktop).
