...
**7-day forecast section on outdoor tasks:** When an outdoor-leaning task (energy=physical/errand, or title matches outdoor keywords like "mow"/"paint the deck"/"wash car") has a due date in the forecast window, the mobile card renders a compact 7-day forecast as its own card section (always visible — not gated on expand). Layout is a centered row of 3 days (larger) above a centered row of 4 days (smaller). Each day shows condition icon, high/low, and wind speed. The task's due date is visually highlighted. Implemented in `src/components/WeatherSection.jsx`.

**"Best days" recommendation in the expanded card:** For the same outdoor tasks, tapping a card (expanded inline quick-edit) renders a "Best days: …" line just above the notes with a sun icon. Up to 3 days are picked by `pickBestDays()` (scored on condition kind, precip probability, wind, and temperature comfort). The recommendation is computed live from the forecast cache, so it stays fresh as the outlook updates. Not written into the `notes` field; lives as a separate UI element so user-typed notes stay clean. The full `EditTaskModal` does not currently show this line — only the quick-expand view does.
