...
### Crisis Tag ("prio", 2026-07-14, migration 040)
A user-configurable label (`settings.crisis_label`, default `prio`; default label seeded in `DEFAULT_LABELS`) that flips a task onto the **crisis path** — the most aggressive treatment in the app. Full spec + decisions: `wiki/Crisis-Tag-And-Impact-Ranking.md`.
...
- **Guardrails:** NEVER auto-applied — the crisis label is excluded from AI auto-tag candidates; Quokka's system prompt (rule 10) only allows it when the user explicitly declares an emergency.
- **Settings (Settings → Notifications → Crisis mode card):** `crisis_label`, `notif_freq_crisis`, `crisis_stale_days`, `crisis_auto_breakdown`.
