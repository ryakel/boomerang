...
**Quokka secret blocklist** (`adviserToolsMisc.js`): the adviser's `update_settings` tool refuses to write, and `get_settings` redacts: `anthropic_api_key`, `openai_api_key`, `notion_token`, `trello_api_key`, `trello_secret`, `gcal_client_secret`, `tracking_api_key`, `pushover_user_key`, `pushover_app_token`. Keep this list in sync when adding new secret-shaped settings.
