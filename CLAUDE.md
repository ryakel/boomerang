...
**"Best days" recommendation in EditTaskModal:** For the same outdoor tasks, the EditTaskModal renders a "Best days: …" line just above the Notes field with a sun icon. Up to 3 days are picked by `pickBestDays()` (scored on condition kind, precip probability, wind, and temperature comfort). The recommendation is computed live from the forecast cache and reacts to in-modal edits to title/energy — so retagging a task changes the recommendation immediately. Not written into the `notes` field; lives as a separate UI element so user-typed notes stay clean.
