...
4. **Verify against the user's data shape, not just the seed's.** The 2026-06-10 streak incident shipped a "verified" fix whose restoration path silently no-oped in prod: the seeded test DB had months of done tasks for analytics to find, the user's post-wipe DB did not. Before claiming a data-dependent fix is verified, ask "what does the user's actual data look like?" (post-wipe DB, pending-only records, empty history) and reproduce THAT shape. And before telling the user "test it now," confirm the deploy actually landed (Docker Builder workflow green + container cycled) — they will screenshot the old bundle and you'll look like a liar.
...
8. **Server runtime modules live in `server/` (2026-07-16 reorg) — new server code goes THERE, and the Dockerfile copies the directory wholesale (`COPY server ./server`).** This retired the old explicit per-file COPY list and its silent-drop trap (a new root module missing from the list crashed prod with `ERR_MODULE_NOT_FOUND` only after deploy). What still needs a Dockerfile edit: a genuinely NEW top-level directory that must ship to prod (`COPY <dir> ./<dir>`). Already covered, no edit needed: anything in `server/`, `migrations/`, `scripts/`, and all `src/*` (ships via the Vite bundle into `dist/`). Dev-only files (`eslint.config.js`, `vite.config.js`, `previews/`, tests, docs) stay OUT of the image. Path conventions inside `server/`: modules import each other as `./x.js`; reaching repo-root resources uses `..` (`../scripts/backup-db.js`, `path.join(__dirname, '..', 'dist')`, `path.join(__dirname, '..', 'migrations')`); the shared `server/aiModels.js` is imported by the client as `../server/aiModels.js` from `src/`. NOTE: older sections of this file say "root module, in the Dockerfile runtime COPY list" — historical phrasing from before the reorg; those modules are all in `server/` now.
