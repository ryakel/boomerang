...
**Rebuild command (Mac):** `npm run ios` (`scripts/ios-rebuild.sh`) = `npm install` + `npm run build` + `cap sync ios` + `cap open ios`. This is the standard "I pulled a branch, put it on my phone" command — it runs `npm install` first so a branch that added a dependency doesn't fail the Vite build with "failed to resolve import" (the trap `build:mobile` alone hits, since it skips install).
...
**Roadmap:** Phase 2 Share Extension (Swift → `POST /api/intake`, reads token from the App Group) → Phase 3 App Intents (Siri/Shortcuts/Action button) → Phase 4 native APNs push + deep links. **All three need the paid Apple Developer Program** (App Groups + Push entitlement are paid-only). Full build guide + Mac steps: `wiki/iOS-Native-App.md`.
