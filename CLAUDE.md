...
- Watch app problems are never diagnosable from the build log — an empty icon set, a stale asset catalog, a missing `CFBundleIconName`, and a provisioning profile that doesn't cover watchOS all end in BUILD SUCCEEDED and collapse into the same two on-wrist symptoms. Run `npm run ios:watch-doctor` and read the bundle. Targets with `GENERATE_INFOPLIST_FILE = NO` must carry `CFBundleIconName` themselves; automatic signing silently falls back to the iOS wildcard profile (`Platform` = iOS/xrOS/visionOS) until the paired Watch is a registered development device, and watchOS then refuses the install. "Registered" means the watch's UDID is filed under the **watchOS** device class in the developer portal — a separate class from iOS — and `-allowProvisioningUpdates` will renew profiles and mint certificates but will **not** register an unknown device; it fails with `Device "…" isn't registered in your developer account` instead. Developer Mode on the wrist is necessary and not sufficient.
