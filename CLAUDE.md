...
**Fixed-schedule cadence (`getNextDueDate` in `src/store.js`, 2026-05-30):** Routine due dates form a FIXED GRID anchored at the routine's `created_at`, NOT its last completion. Completing early or late never shifts the series — "every Monday" stays Monday, "the 5th" stays the 5th. `completed_history` only marks which grid slot has been satisfied (the next due is the first grid slot after the slot containing the most recent completion); it never re-bases the grid. Daily is special-cased (fires every calendar day; due today unless already done today → tomorrow). A missed cycle surfaces as one overdue task, not a pileup; completing it advances to the current slot. **Behavior change from the old completion-anchored model:** before, completing N days late pushed the next occurrence N days later (drift); now it doesn't. Anchor is `created_at` (no per-routine anchor UI yet) — weekly day is controlled by `schedule_day_of_week`, monthly day-of-month by the creation day.

**Weekday anchor (`schedule_day_of_week`, 0=Sun … 6=Sat):** optional column added in migration 017. For **weekly** routines it's folded into the grid origin so every slot lands on that weekday. For month-scale cadences (monthly/quarterly/annually/custom-months) each grid point snaps forward to the first occurrence of that weekday. Examples:
- Weekly + Fri → every Friday on the fixed grid (completing off-day doesn't move it)
- Quarterly + Sat → first Saturday on/after the 3-month grid point (may drift up to 6 days — intentional, "air filter on a weekend" is the use case)
