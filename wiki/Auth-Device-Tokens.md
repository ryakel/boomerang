...
**Status:** Phase A built (2026-07-25, shipped v2.38.0). Phase B native half built (2026-07-26, BoomerangKit, verified on-device — the Run check returned `server_pending`); server-side verification built the same day (`server/appAttest.js`) — `/attest` now verifies for real against the pinned Apple root.
...
**Native half (BUILT 2026-07-26, `BoomerangKit/AppAttestClient.swift`):** the full client flow is implemented — fetch challenge → `generateKey` (persisted in the shared Keychain, cleared if `attestKey` poisons it) → `attestKey` over `SHA256(challenge)` → POST `/api/auth/device/attest` with `{key_id, challenge, attestation, device_id}`. Run it from Settings → Data → Devices & security → **App Attest → Run check** (native shell only). **Verified on-device 2026-07-26**: the check returned `server_pending` against the then-501 stub, proving the native side end-to-end on real hardware — which unblocked the server verifier the same day.

**Server verifier (BUILT 2026-07-26, `server/appAttest.js`):** dependency-free — a minimal CBOR decoder (RFC 8949 subset), a small DER walker for the nonce extension, and `node:crypto X509Certificate` for the chain. Verification order per Apple's docs: CBOR decode → cert chain to the **pinned Apple App Attestation Root CA** (`server/appleAppAttestRootCA.pem`, fetched from apple.com/certificateauthority, valid to 2045) → nonce == SHA256(authData ‖ SHA256(challenge)) matched against credCert's `1.2.840.113635.100.8.2` extension → keyId == SHA256(uncompressed P-256 point) → authData (rpIdHash == SHA256(App ID), counter 0, aaguid `appattest`/`appattestdevelop`, credentialId == keyId). Allowed App IDs default to the two known team-scoped bundle ids; override with `APP_ATTEST_APP_IDS` (comma-separated). On success the device row stores `{key_id, public_key, counter, environment, attested_at}` (public key + counter reserved for future assertion checks — counter regression = cloned key). A FAILED verification from an authenticated caller fires the `attest_failure` security alert (loud category). Challenges stay single-use/5-min; reuse is an ordinary 400 retry, not an alert.

**Testing:** `scripts/appattest.test.mjs` builds a SYNTHETIC attestation with openssl (test root passed via the verifier's `rootPem` override; leaf carries the real Apple nonce OID) — every verification step is exercised in CI except the identity of Apple's actual root, which the on-device Run check covers (expected outcome now: **`verified`**). Gotcha for future tests: node 22's `test(..., { skip: null })` REPORTS the test as skipped while still running it — always pass `skip: reason ?? false`.
