...
- [x] ~~**MarkdownImportModal** in v2.~~ Landed 2026-05-09. Reachable from More menu → "Import from markdown." Paste or upload a `.md`/`.txt`/`.markdown` file → preview parsed tasks (bullet lists + checkbox lists + section headings as group labels) → toggle which to import → bulk creates via `addTask({ title })`. ExtendModal + FindRelatedModal **explicitly skipped** — both are superseded by existing v2 flows: Extend's date-preset UX is covered by EditTaskModal's date input, and FindRelated's Notion search is covered by the inline Notion search/link/create in EditTaskModal Connections (PR #36).
...
- [ ] **Smart follow-up sequences (post v2 → main).** Completion-triggered task chains — orthogonal to routines (time-triggered) and checklists (single task). Use case: "after I clean the floors, auto-clean the mop immediately, then in 30 min empty the tanks, then in 2 days put the dry containers back." Each step has its own offset, energy/title/notes, and only spawns when the previous step is completed. Two implementation shapes considered: (a) standalone "Sequence" primitive sibling to Routines (cleaner mental model, third top-level concept), or (b) `follow_ups: [{title, offset_minutes, energy, …}]` array on Tasks AND Routines that walks recursively as each step completes (less menu surface, conflates two configurations). User leans toward bolting onto Routines. Open questions: cancel semantics if a mid-chain step is deleted (probably: chain stops, no auto-spawn), snooze behavior (snooze just delays current step's due date, spawn-on-complete still fires when finally completed), Quokka tool surface for creating/editing sequences. Spec before building.

- [ ] **Better Logs filter UX (post v2 → main).** Current filter chips are hand-curated string matches against bracketed prefixes (`[Gmail]`, `[GCal]`, `[Push]`, `[Email]`, `[DB]`, `[SSE]`, `[SYNC]`). Adding a new server module today means editing the chip list AND remembering to use a new prefix in the module's `console.log` calls. Better shape: source the available filters from the actual log stream (auto-discover unique `[Tag]` prefixes from the last N log lines, render as chips), OR move toward structured logs (`{level, tag, msg}` objects emitted server-side, filter by tag enum) so chips reflect reality without hand-maintenance. Probably structured logs for stability + auto-discovery as a fallback for legacy unstructured lines. Combined-tag chips like the current `Google` chip stay as a manual override on top of the auto-discovered set.

- [ ] **Web push deprecation trial — post v2 → main merge.** Once v2 ships to production and Pushover has soaked for a couple of weeks of real use, revisit whether web-browser push is still pulling its weight. Engagement Analytics' tap-rate / completion-rate per channel will surface this. Decision criteria: if Pushover catches every (priority × type) combo at parity or better, hard-deprecate web push (remove `pushNotifications.js`, SW push handler, VAPID lifecycle, `push_subscriptions` table, subscribe/unsubscribe UI, inline-action endpoints, engagement-analytics push column — ~250-400 LOC net delete). If anything slips through Pushover (Android browser users, desktop, etc.), keep web push as the fallback. Not deciding now; logging the trial so it doesn't get forgotten.
