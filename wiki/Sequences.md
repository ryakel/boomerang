...
- **PR 5 — Quokka tools (SHIPPED).** Four atomic tools on routine `follow_ups` arrays, all in `adviserToolsTasks.js`:
  ...
  - **Bugfix 2026-07-04 — same-plan `create_routine` → `add_follow_up` chaining.** A plan that creates a routine and chains a follow-up onto it in the same turn used to fail at commit with `Routine not found: <id>` — `create_routine` only generated its real id inside `execute` (commit time), so the staged response the model saw had no real id to put in the follow-up's `routine_id`, and the model fabricated one. Fixed by giving `create_routine` the same `preStage` id-pre-stamping hook `create_task` already has (real id returned in the staged response), plus a `stagedValidate` on `add_follow_up`/`spawn_routine_now` that accepts either a real routine id or a same-plan staged `create_routine`'s id — surfacing a clear correction at stage time instead of a commit-time rollback.
...
- `src/components/RoutinesModal.jsx` — `FollowUpStepRow` (new), `RoutineForm` state + UI, `handleSubmitForm`
- `src/components/RoutinesModal.css` — `.v2-followups-*`, `.v2-form-section-hint`
