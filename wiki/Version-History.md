...
- docs(ios): the share sheet's "Add to" row is below the fold, not broken [XS]
  ...
  - **Owed fix recorded, not shipped:** not-a-bug is not the same as shipped — "I'm not gonna remember to scroll three months from now" (owner). The agreed approach is to **auto-scroll rather than change behaviour**: keep the keyboard and focus, walk the view hierarchy in `viewDidAppear` for the enclosing `UIScrollView`, and scroll the config row into view. Public properties only, no private selectors. Guarded so a hierarchy change degrades to exactly today's behaviour rather than a blank sheet, and it needs an on-device check since there is no Mac here. Rejected alternatives are recorded too (keyboard-down costs a tap on every title edit; destination-in-title labels the row without reaching it; picker-first interrupts before you can see what you are sharing). **To be bundled into the next Share-Extension-touching change**, per the owner.
