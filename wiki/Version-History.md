...
- fix(digest): away actually suppresses, and the notification says something [M]
  - Reported from the road with a screenshot of the morning push — title **"Today"**, body **"Caulk around windows"** — and two sentences: *"Your away mode doesn't work and this notification sucks."* Both were true, and they were separate bugs.
  - **The away bug was a stale cache, not the suppression.** `isNotifiable()` was suppressing correctly the whole time — verified directly against the DB: window active, `isAway → true`, notifiable count **0**. But `assembleDigest()` memoises on `{date, version}`, and `version` is the TASK version. The away window lives in the `vacation_window` `app_data` carve-out (deliberately — see `vacationWindow.js` for why it can't ride the settings blob) and switching it on bumps nothing. So a digest built earlier that morning — by the app simply *opening* — was served for the rest of the day, and the scheduled push sent the copy that predated the window. Suppression was right; the thing that read it was looking at yesterday's answer. The cache key now includes the window's own identity, so any writer invalidates it, not just the endpoint.
  - **`settings.timezone` doesn't exist.** Three away call sites (`isNotifiable`, the streak freeze, the repair planner) plus the `/api/vacation` response read it; every other caller in the codebase reads `user_timezone`. They were silently falling back to `America/Chicago`, so the window's day boundaries were evaluated in the wrong timezone — which for someone who has just flown somewhere ahead of Chicago can mean the first morning away isn't "away" yet. Not the cause of this incident, but a live second way for the same feature to fail quietly, and it fails exactly when you're travelling.
  - **The notification.** Title "Today" said nothing — every notification this app sends is about today — leaving a bare task title as the body, which reads as an instruction rather than a brief. Now the title carries the count and the reason: `3 due today`, `Today's three`, `Nothing due today`, or `🏝️ Away`.
  - **And it stops lying while away.** The away statement was computed *after* the push copy and only ever reached the expanded text, so the push for a suppressed day said *"A quiet day — nothing scheduled"* while the window held ten tasks. The away line is now computed first and the push says `Holding 10 tasks until you're back. Critical still gets through.` — the failure mode this feature was explicitly designed against was live in the one place most days are read from.
  - Verified end to end against a live server: warm the cache → switch away on → the very next digest flips to the away copy → switch off → it flips back. `npm test` 286/286 + smoke, eslint 0 errors. Server only — **no rebuild**.
...
- fix(throw): the sheet stops bouncing when you tap a chip [S]
  ...
  - **The bouncing:** every chip in the sheet is a `<button>`, and a button takes focus on tap. That blurs the title input, iOS closes the keyboard, the sheet drops; tapping back into the text box reopens it and the sheet jumps up again. `preventDefault` on `mousedown` for the mode chips and the day chips suppresses the focus transfer while leaving the click intact — the keyboard never closes, so the sheet never moves.
  - **The floating:** `.bm-sheet` reserves `env(safe-area-inset-bottom)` in its bottom padding for the home indicator. Once the keyboard is up the home indicator is *behind the keyboard*, so that reservation protects nothing and paints ~34pt of dead sheet-coloured space between the action button and the keyboard — which is what made the sheet look like it was hovering mid-screen instead of resting on the keyboard. `.is-lifted` drops the padding to 14px while translated.
  - The keyboard-offset math itself was checked and is **not** the bug: the sheet sits in a `position: fixed; inset: 0` flex-end container, so its untransformed bottom is exactly `window.innerHeight`, which makes the existing formula algebraically identical to measuring the element's own rect against the visual viewport. Left alone rather than rewritten to look busy.
  - Verified in a browser: focus stays on `.bm-throw-input` through taps on Task / Reminder / Note / Tomorrow, and the chips still act (heading and selection both update). The padding half is verified by measurement (24px → 14px, and 58px → 14px once a real safe-area inset exists) rather than on-device — there is no iOS here.
- style(throw): Reminder is a mode, not a chip in a new row [S]
  - Four corrections in one screenshot, all correct: *"Reminder, not remind"*; *"there is no other emoji usage in buttons so why here"*; *"why the fuck would you put this in a new fucking row at the bottom rather than with task and notes at the top"*; and restore the bottom spacing.
  - The Throw sheet already had a control that says "these are alternatives to each other" — the Task | Note toggle at the top. A reminder is a third alternative, so it belongs **there**, not bolted on as an extra chip in an extra row below the day chips. Now **Task · Reminder · Note**, and the row under the title becomes whatever that mode needs: day chips for a task, a date-and-time picker for a reminder, nothing for a note. Sheet height and the gap above the action button are unchanged across modes (the picker carries the chip row's exact 10px bottom margin).
  - The clock icon is gone. No other button in this sheet carries an icon; that one was decoration justifying its own existence.
  - Headings and action verbs follow the pattern the Note mode already established: *Set a reminder* / *Set it* beside *Throw a task* / *Throw it* and *Leave a note* / *Leave it*.
  - Three chips plus the heading do not fit a 390pt row at the full chip size — measured, not guessed: the heading wrapped to two lines and made the sheet taller. The mode chips take the compact scale already in the design language (a secondary control), the heading gets `white-space: nowrap`, and the row gets `flex-wrap: wrap` as the large-accessibility-text fallback so it degrades to two lines rather than letting the heading hit its min-content floor and paint over the chips (the same overflow shape as the theme/away-window bug).
  - Verified: reminder mode → `remind_at 2026-08-11T19:30`, `due_date null`; switching to Reminder and back to Task leaves `remind_at null` with the day chip intact, so a seeded time can't leak into a plain capture. `npm test` 286/286 + smoke, eslint 0 errors. Web only — **no rebuild**.
- feat(loops): a reminder that becomes a loop, decided out loud [M]
  - The last piece of the scoped reminder work: *"If we convert a reminder to a loop, compare the start date of the loop with the reminder date… let the user know with a little just-in-time nudge"*, plus the quick-edit recurrence affordance (*"click recurrence in the quick edit as a checkbox and then have it copy what has already been entered"*).
  - **Repeats chip** in the quick editor — cadence row (Daily / Weekly / Monthly / Quarterly / Yearly / Every N), then *Make it a loop*. Converts **in place** from what's on screen; the full editor is no longer a detour for the commonest case. A task that already belongs to a loop says so instead of offering to make a second one.
  - **The absorption rule** (`src/loopAbsorb.js`, pure, 12 tests): the loop absorbs the reminder when the loop's FIRST CYCLE already covers that moment — it takes the reminder's time of day as `trigger_time`, flips `remind` on, and rings every cycle. Outside that window the reminder stays a standalone one-off, because it is asking for something the loop won't do yet. The sharp case is "remind me Tuesday, and weekly after that" where the grid puts the first spawn on Thursday: absorbing there would silently move the ring two days.
  - **A pending reminder is NOT cleared on absorb**, which is the opposite of the obvious implementation. The converted task stays active carrying `routine_id`, and the spawn pass skips a routine that already has a non-done instance — so the loop will not mint a first occurrence to carry the alarm. Clearing would have silently eaten the one ring the user was standing in front of. Only an already-fired reminder is dropped; its time of day still survives as the loop's trigger.
  - **`getNextDueDate` stays the only cadence grid.** The pure module takes the first spawn as a PARAMETER rather than recomputing weekday snapping / day-of-month rules / the series-start walk. A second grid that drifts from the first would be a worse bug than the one being fixed.
  - The nudge is shown at the decision point in **both** editors and updates live as the cadence changes — switching Daily→Weekly flips it from "the loop takes that time from here on" to "it stays as a one-off". Both editors now share ONE conversion handler in `AppV2`, so the rule cannot drift between the two ways to do the same thing.
  - Verified in a browser against a live server, both paths: quick-editor convert produced `trigger_time: "10:43"` + `remind: true` with the pending `remind_at` intact; the full editor's weekly default correctly declined to absorb and said so. `npm test` 286/286 + smoke, eslint 0 errors, `npm audit` clean. Web only — **no rebuild**.
...
- chore(git): stop cutting branches from the branch that was just merged [XS]
  - The same self-inflicted problem three times in one session: a new branch cut from the previous local feature branch opened its PR as `mergeable_state: "dirty"` with **no CI run at all**, and had to be rebased before it could merge.
  - Cause, written down so nobody re-derives it: `dev` merges use `merge_method: "rebase"`, which **rewrites the SHAs** — the commits on `dev` are not the commits that were pushed. A branch started from the old local branch therefore carries the pre-rebase SHAs and replays already-merged work. GitHub calls that a conflict. It is not one; it is stale SHAs, and resolving it as a conflict would duplicate the merged commits.
  - Fixed as an invariant in `CLAUDE.md` plus `scripts/new-branch.sh` so it needs no memory: `sh scripts/new-branch.sh <thing>` fetches and cuts `claude/<thing>` from the current `origin/dev`. The recovery is recorded too — `git rebase --onto origin/dev <last-already-merged-sha> <branch>`, never a conflict resolution.
...
- fix(reminders): three defects found reviewing the uncompiled EventKit plugin [S]
  ...
  - **Would compile and duplicate tasks:** the link was reported back only when no id had been supplied, but a supplied id that no longer RESOLVES (deleted on device, iCloud still populating) also creates a new reminder. The server would keep the dead id while the fresh reminder went unlinked — and the next sync would unlink the task *and* import the orphan as a second one. Now tracked with an explicit `isNew` rather than inferred from the input.
  - **Would compile and mislead:** `try? store.saveCalendar` swallowed a failure and returned the unsaved calendar anyway, so every later write failed with something unrelated to the actual cause. Now returns nil and the caller rejects with the real reason.
  - Verified against the vendored Capacitor sources rather than from memory: `CAPPluginCall` does conform to `JSValueContainer` (so `getArray(_:JSObject.self)` exists), and **`NSNull` conforms to `JSValue`** — so a JSON null decodes cleanly on the way IN, and the watch's `NSNull` trap does not repeat on this path. `npx cap sync ios` runs clean.
  - `Package.swift` is regenerated by `cap sync` (deployment target, plugin paths) and was deliberately NOT committed here — it is CLI-managed, both deploy scripts run `cap sync` themselves, and a platform bump does not belong in a feature PR.


- feat(reminders): two-way Apple Reminders sync — iOS rings the alarm [L]
  - Requested as "I want reminders and the ability to sync reminders with the iOS app". Two features share one link, and the pairing is the point.
  - **Apple owns the alarm.** `due_date` is a DATE with no time anywhere in the schema, so "interrupt me at 3pm" did not exist. New `tasks.remind_at` is a MOMENT, kept deliberately separate from the due DAY — most tasks want a day and would be made worse by being forced to pick a time. The alarm is an `EKAlarm` on an `EKReminder`, so it reaches the Lock Screen, watch and CarPlay **with no Boomerang send path at all**. That is what lets per-task reminders exist without reopening the flood the 2026-07-24 reshape deleted: the surviving-pings list is untouched, because nothing new sends.
  - **The same list is the Siri inbox.** "Hey Siri, remind me to call the roofer" already works perfectly and lands in Reminders; pulling that list in makes it a Boomerang task (`capture_source: 'reminders'`). After weeks of App Intents phrase-matching, voice capture works by not fighting it.
  - **The merge is pure and lives on the SERVER** (`server/reminderMerge.js`, 23 tests). EventKit is device-local so the reading and writing must happen on the phone, but the DECIDING does not — `BoomerangReminders.swift` lists what it sees, posts it, and writes back exactly what it's told, the same reasoning as the watch holding no credentials. A merge running on-device would let two phones resolve one conflict differently, with no way to test either.
  - Rules that earned tests. 3-way against a `reminder_shadows` baseline, because a two-way diff cannot tell "I renamed this in Boomerang" from "I renamed it in Reminders" and eats one every sync. **Boomerang wins a genuine conflict** (it is the system of record) — but it says so in `held`, because a discarded edit must not vanish silently. **Completion is the one exception and travels from either side**: losing a completion re-rings an alarm for finished work, which is precisely how a reminder system teaches you to ignore it. Times compare as INSTANTS with a 60s tolerance — a raw string compare against the phone's `NSDate` round-trip reports a change every sync and starts a write ping-pong. A reminder deleted in Apple's app **drops the LINK, never the task**; a swipe in Reminders must not destroy work that lives here. Losing >50% of linked reminders is a bad read, not a mass delete (same guard, same reasoning as the Trello list sync).
  - A failed EventKit fetch **rejects rather than resolving empty** — an empty list is indistinguishable from "everything was deleted", and resolving it would hand the merge a clean sweep to act on.
  - Verified end-to-end against a live server: a reminder with no task imported as a task with `capture_source: reminders`; a task with `remind_at` emitted as a CREATE; a re-sync of the agreed state produced **zero writes** (no ping-pong); completion in Reminders set `status: done` + `completed_at`; a vanished reminder unlinked the task and kept it, reporting `remote_deleted`. `npm test` 246/246 + smoke, eslint 0 errors, `npm audit` 0.
  - **The native half is unbuilt.** Swift, `Info.plist` and the Xcode target wiring are committed, but nothing reaches Reminders until the app is rebuilt on a Mac — Capacitor does not auto-discover compiled-in plugins, so `BoomerangReminders` is registered explicitly in `BoomerangViewController` alongside `BoomerangNative` (the 2026-07-16 trap). Requires **Full Access** to Reminders; iOS 17's write-only tier cannot read and so cannot support a two-way sync at all.


- feat(ai): the model pickers ask the providers instead of a baked-in list [M]
  - Reported as *"I can't see fable or opus 5 or any of the new ChatGPT option"*. The cause: `SettingsModal.jsx` **imports `MODEL_CATALOG` from `server/aiModels.js` directly**, so the offered list was compiled into the client bundle. A model released after a build was unreachable until someone hand-edited the array and shipped — the catalog was last touched in July.
  - New `GET /api/ai/models` asks each **configured** provider what it actually serves (`GET /v1/models` on both), merges the answer over the catalog, and caches it in `app_data` for 6h (`?refresh=1` forces). New pure module `server/aiModelDiscovery.js` holds the merge, the filters and the normalizers; the network half is one function. 23 tests.
  - **The catalog keeps the jobs it is good at.** Labels and pricing still come from it, because a hand-written "Claude Sonnet 5" beats a raw id and a discovered model has no price — `estimateAiCost` already returns null for unpriced models, so the usage dashboard shows *no* cost rather than a wrong one. **Routing is untouched**: `resolveModelRef`/`resolveTierModel` remain the only thing deciding where a call goes, per the CLAUDE.md invariant. Discovery decides only what the picker may OFFER.
  - Three rules earned their tests. **Catalog entries are kept even when a provider omits them** — a filtered response, an outage or a key without access to one model must not delete a model the user has already selected, and an empty discovery degrades to exactly the catalog rather than to an empty picker (the same "failed renders as empty" trap the Share Extension hit). **OpenAI's `/v1/models` is a kitchen sink** — embeddings, TTS, whisper, dall-e, moderation and realtime all sit alongside chat models and are filtered by name; verified zero leaked. **Dated snapshots collapse into their undated alias** when both are offered (`gpt-5.5-pro-2026-04-23` → `gpt-5.5-pro`), which took the live list from 79 to 49; a dated id whose base is absent is kept, because then it's the only route to that model, and Anthropic's different date shape (`claude-haiku-4-5-20251001`) is deliberately not matched.
  - Sorted newest-first within a provider so a just-released model is at the top. Per-provider status (`ok` / `error` / `not_configured`) rides the response — a provider that FAILED must not look like a provider with nothing new, or the picker silently goes stale again invisibly. An Anthropic-only setup never sees an OpenAI error it can do nothing about.
  - Verified live end-to-end: 49 OpenAI models discovered and rendered in the picker (newest first, no non-chat leakage), Anthropic falling back to the catalog with no key present, `Custom…` still available for pinning anything at all.

- fix(settings): a missing endpoint no longer reads as three unrelated bugs [S]
  - Three separate reports from the phone — Server version stuck on `…`, native push failing with *"Server rejected the token (200)"*, Away mode reading *"Unavailable — The string did not match the e…"* — were **one mechanism**. `app.get('{*path}')` serves `index.html` for any unmatched GET, so an `/api` route the running server doesn't have comes back **200 with HTML**. `r.json()` then throws, and on WebKit the parser's own message is `The string did not match the expected pattern.`, which is what surfaced to the user. Each caller disguised it differently: the health fetch swallowed it into a bare `.catch(() => {})` and sat on its placeholder forever, and `nativePush` swallowed it into `{}` and reported a sentence that blames a healthy token and names a success status.
  - **Server:** a terminal `app.use('/api', …)` ahead of the static/SPA block returns an honest JSON `404` for every method, and logs the route. A client older or newer than its server now says so instead of pretending. Verified: unmatched GET and POST both return `404 application/json`, real routes still return JSON, non-API paths still serve the SPA.
  - **Client:** one `readJson()` in `apiConfig.js` is now the only way an `/api` body is read on these paths — it distinguishes non-ok, HTML, and unparseable, and never lets a parser message reach the UI. The health row resolves to `unreachable` rather than `…`, so "can't reach the server" stops rendering identically to "still loading" on the one row whose whole job is to say what you're talking to.
  - **This does not fix the underlying condition**, and it is not supposed to: the phone was talking to something answering `200 + HTML` for `/api/vacation`, `/api/apns/register` and `/api/health`, while `/api/tasks` worked. That is a deploy/routing problem, and the point of these changes is that it now announces itself instead of appearing as three unrelated feature bugs.

- fix(settings): row labels no longer paint over their own controls [XS]
  - Reported with screenshots: the Theme label overlapping its segmented control, and "Away window" overlapping its status text. One cause — a flex item's floor is its **min-content width**, so a label that hits that floor stops shrinking and starts overflowing, on top of whatever trailing control shares the row. `.v2-set-row-value` had `overflow: hidden` discipline; `.v2-set-row-label` had none.
  - `min-width: 0` + `overflow-wrap: anywhere` on the label makes the worst case a broken word instead of an unreadable overlap. The Theme row is additionally now `stacked` like Mode — the segment is `width:100%` up to 320px, which leaves nothing for a label beside it at phone width, which is exactly the condition §2.3 defines `stacked` for. Measured at 402px: label and segment no longer share vertical space, overlap 0.
  - `StatusRow` gains `detail` for the failure case. The trailing value is nowrap + ellipsis, so a diagnostic sentence put there got cut mid-word — the one row explaining what broke was the one row you couldn't read. The value stays a short state word (`Unavailable`) and the sentence wraps below it.
  - Verified in a real browser at 402×874 against a seeded server, including an end-to-end reproduction of the reported failure (`/api/vacation` forced to answer 200 + HTML): message legible in full, overlap 0.

- docs(wiki): write down the four parked items before the trip [XS]
  - New `wiki/Parked-20260801.md` — the work identified during the watch-connectivity and away-mode session and deliberately not built, recorded while the verification is still fresh rather than re-derived next week. Each entry carries what already exists (checked against the tree, with file references), what is actually missing, **the decision that has to be made before coding**, and the failure mode.
  - The useful finding is how uneven they are. **Pin to today is S and ready** — `POST /api/tasks/:id/commit` and `/uncommit` are merged, tested, ceiling-enforcing and already called by Siri (`BoomerangIntents.swift:262`); the client never calls them (grep for `/commit` in `src/` finds only Quokka's unrelated `/api/adviser/commit`). The only open question is where the affordance goes in a Kept row that already has two controls. **Packages-drive-due-dates is better-founded than assumed** — `packages.eta`/`delivered_at` exist and `signature_task_id` is a working package→task link precedent, so the pattern generalizes rather than needing invention; migration-049's provenance columns are the right home for the shifted date. **Geofencing** is mostly ceremony (Mac build session, entitlement) over the already-validated-but-dead `location_json`. **Calendar away-detection** is last on purpose: only the inference is missing, and both failure modes are invisible to the user — so it must propose a window, never write one.
  - Two of the four (pin-to-today, geofencing) are slices of `wiki/Feature-Recs-20260725.md` recs 1 and 5; the new page records what has since been verified rather than restating the product framing. The other two appear nowhere else. Indexed at the top of `wiki/UPCOMING_FEATURES.md`, matching how the 2026-07-25 recs are surfaced. Docs only — no code changed.
...
- fix(watch): scrub NSNull from every WatchConnectivity payload [M]
  - **A code audit found a prime suspect for the 2026-07-26 "Payload could not be delivered" failure, in the payload itself** — the one place the original investigation never looked, because the error reads like a transport problem. Every data-bearing reply embedded raw `/api/today` JSON; `todayPayload()` emits `timer: null` unconditionally plus per-task nulls; `JSONSerialization` turns JSON null into `NSNull`; and a WatchConnectivity dictionary may contain only property-list types, which `NSNull` is not. **100% of today/complete/commit replies were invalid** — matching "every request fails" with no dependence on launch, pairing or signing, which is exactly why those all checked out healthy. The only valid replies this bridge could ever produce were the pure-string ones.
  - `plistSafe()` recursively drops null-valued keys at the single choke point (`fetchToday()`), covering both reply sites and the context push. Behavior-preserving: the watch reads all affected fields with `as?` optionals. The context push's `try?` — which was silently swallowing the identical rejection on the wrist-raise path — is now a logged `do/catch`. Breadcrumb `NSLog`s cover activation/pairing/receipt (a background launch used to leave no trace at all), and `WatchStore.send()` drops a pre-activation send instead of racing `onAppear` against activation (a real bug found in the same audit, though its error signature differs).
  - **Unverified — no Mac, no wrist here.** The wiki's OPEN section now carries the discriminating on-device test to run FIRST (VPN off → tap Refresh: a string-only reply arriving proves the transport and convicts the payload) and the honest caveat that iOS's exact error mapping for an invalid reply isn't provable from the repo. Status: fix shipped, unresolved until the wrist says so.
...
- docs(ios): mark the superseded auto-scroll plan as reversed [XS]
  - Walking the log hit the superseded intent before the correction: `ad132c1` recorded auto-scroll as the **owed fix**, and `c2e6fe8` two commits later explained it was rejected in favour of owning the layout. Both entries were accurate about their own moment, so a reader taking the first at face value would go looking for a scroll hack that was never written.
  - The `ad132c1` bullet now carries an explicit **SUPERSEDED the same day** marker, the owner's pushback that reversed it, and the line that matters: **nothing in that bullet was implemented.** Left in place rather than rewritten — it is the record of a decision made and reversed, which is worth keeping, just not worth mistaking for a plan.
  - `wiki/iOS-Native-App.md` re-tensed: the section heading read "The 'Add to' row **is** below the fold" in the present tense after the rewrite had removed the cause, so a skim of the headings said the opposite of the truth. Now past tense and flagged historic, with the diagnostic lesson kept — that is the part worth an evening's tuition.
  - Docs only.

- refactor(ios): the Share Extension owns its layout now [M]
  - The destination row was below the fold because `SLComposeServiceViewController` has a fixed layout — nav bar, text view, then configuration items — and focuses the text view on appear, so the keyboard pushed the row out of sight. Not a bug, but not shipped either: "I'm not gonna remember to scroll three months from now" (owner).
  - **The auto-scroll fix was rejected, and correctly.** Walking a view hierarchy Apple doesn't expose, in `viewDidAppear`, guarded against version drift — all to work around a constraint we were free to drop. The template offers no ordering hook and no reposition API, so the layout was never ours to arrange; owning it is the smaller idea.
  - `ShareViewController` is a plain `UIViewController` now: header (Cancel / title / Add), **destination row directly beneath it, above the text**, then the text view and URL preview. Nothing to scroll. The keyboard still focuses the text — the row is above it — and the card lifts clear via `keyboardWillChangeFrame`. Replaced `contentText` with a `UITextView` + placeholder label, `isContentValid()` with `refreshValidity()` on `textViewDidChange`, and the config-items table with one button whose **whole row** is the tap target (the mistake settings PR6 had to fix — evidently easy to make twice).
  - Folded in: the two near-identical POST paths collapsed into one `send(path:…)`. They had already drifted — only one trimmed a trailing slash — and both now carry the 10s timeout.
  - **Fixed a docs regression I introduced hours earlier in the same session:** the "below the fold" note (`eadbb7b`) had eaten the `## Phase 3 — App Intents` heading, silently merging two phases in `wiki/iOS-Native-App.md`. Caught by a failing search for the heading, not by reading. All eight Phase headings verified present.
  - **Unverified Swift.** No Mac and no Swift toolchain here, so this is reviewed, not compiled. Checked against the compiler's fragile spots — `NSError` has no two-argument initialiser (that one was a real error, caught in review), and `UITextView.text` is `String!` so it goes through `currentText` everywhere. Expect a fix or two on the first build.

- feat(vacation): Away mode — a real away window, with a UI [L]
  - **The vacation feature was real and it regressed.** An old commit (`9ef70a4`, "Add vacation duration picker") shipped a control; a later UI rewrite dropped it and left the settings keys plus the streak logic reading them, wired to nothing. `wiki/Configuration.md` kept advertising a "duration picker (3, 5, 7 days, or custom)" the whole time. Verified this was NOT the 2026-07-28 settings rebuild: all seven of those commits contain zero `vacation` references. So "I already have a vacation rule" was true, historically — which is why the owner and I disagreed about whether it existed.
  - **It does not go back in the settings blob.** That sync is whole-blob last-writer-wins, and `vacation_mode: false` ships in `src/store.js` defaults, so any client with unhydrated localStorage pushes an explicit `false` and switches the window off mid-trip — the `pushover_open_native` failure, third occurrence. `preserveAbsentSettings()` cannot help: it protects a key a stale client OMITS, and this one is always present. Here the failure is also *invisible*: a reverted window resumes every notification it was set to silence, while you are away and not looking. So it gets its own `app_data` carve-out (`vacation_window`) with dedicated `GET`/`POST /api/vacation`, per the `add-setting` skill's decision tree. Legacy blob keys are read once as a fallback, never written.
  - **`server/vacationWindow.js` is pure** — no db, no network, and the clock is always injected. Days are `YYYY-MM-DD` calendar labels rather than instants, because "away Tuesday to Friday" is a statement about days and an ISO timestamp would make the boundaries depend on what time of day you opened the app. An end before the start drops the end rather than inverting the window (inverting would suppress a range nobody asked for); `windowDays()` is bounded at 400 so a `0202-01-01` typo can't ask for 700k days. **23 tests, weighted toward the windows that must NOT suppress** — inactive, not started, already ended, malformed.
  - **Suppression is a further filter inside `isNotifiable()`, never a second parallel gate** — that function stays the single answer to "may this task nag". Crisis tasks are exempt: the washing machine does not care that you are in Wisconsin.
  - **Retroactive by design.** A start date in the past is accepted, because the case that actually happens is realising on the way home that you never set it. The Notifications row states the dates (`Jul 27 – Jul 31`) rather than "On", and the always-visible description switches to "Suppressing now…" while active — invisible muting is the hazard, so it is readable without opening anything.
  - **Verified live, twice.** The first digest check was a **false positive**: `GET /api/digest/today` ignores `?force=1` and is cached per (day, data-version), so toggling the window (an `app_data` write) left the version unbumped and both reads returned the same cached digest. Re-run with a task write between them: window off → digest carries "Washing machine leaking" *and* "Mow the lawn"; window on → only the crisis task. Browser-verified the UI: row summary `Off` → `Jul 27 – Jul 31`, page title, the suppressing status row, and a retroactive window set through the date inputs landing as `started_at` two days in the past with `away_now: true`, `days: 5`. No page errors.
  - `wiki/Configuration.md` corrected — the stale Rings-and-Goals entry now points at the real control and admits it advertised a missing feature. ESLint 0 errors, build ✓, `npm test` 174/174 + smoke.
  - **Not yet built: the bulk date repair.** This stops the nagging and freezes the streak; it does not yet clean up the dates that went overdue while you were away, which is the owner's actual pain. That needs a migration for `due_date_original` provenance (packages self-delete, tasks must carry their own provenance) and is the next piece.
...
- docs(ios): the share sheet's "Add to" row is below the fold, not broken [XS]
  ...
  - **Owed fix recorded, not shipped:** not-a-bug is not the same as shipped — "I'm not gonna remember to scroll three months from now" (owner). The approach recorded at the time was to **auto-scroll rather than change behaviour**: keep the keyboard and focus, walk the view hierarchy in `viewDidAppear` for the enclosing `UIScrollView`, and scroll the config row into view, guarded so a hierarchy change degraded to today's behaviour. Alternatives rejected then: keyboard-down (costs a tap on every title edit), destination-in-title (labels the row without reaching it), picker-first (interrupts before you can see what you are sharing).
    - ⚠️ **SUPERSEDED the same day** by `refactor(ios): the Share Extension owns its layout now` (below). The owner pushed back on the auto-scroll — *"rather than trying to do something happy and stupid. Why is it below the frame that has to be scrolled to?"* — and was right: the row's position was never ours to arrange while we used `SLComposeServiceViewController`, so the walk was working around a constraint we were free to drop. **Nothing in this bullet was implemented.** It stays as the record of a decision made and reversed, not as a plan.
...
- docs(ios): Siri voice is gated behind a prompt that tapping never reveals [XS]
  ...
  - Docs only — no code changed.
- docs(ios): design "away mode" — the dangerous half of location awareness [XS]
  - Owner request (2026-07-29), correctly flagged by them as needing care: detect being away from home and stop home tasks nagging; possibly defer them by moving due dates. This note exists to say *which* trouble.
  - **The failure that matters: a false "away" silences your life, and silence is unfalsifiable.** Phone off, Always authorization downgraded, reduced accuracy granted, a region event simply not delivered — any of these mute everything home-shaped, and from the inside that is indistinguishable from having nothing due. Same shape as `last_sync_error` and tonight's share-sheet fetch: an absent signal must never render as a negative state. The digest has to SAY "12 home tasks suppressed — you're away", never just show a short list. Exit and entry are also not equally reliable — failing to clear on return means staying muted at home.
  - **Never auto-change due dates for this.** Suppression is reversible and leaves data untouched; rewriting dates mutates recorded intent on a guess about location, and a misfire that rewrites forty dates isn't recoverable by turning the feature off. Bulk deferral belongs as an explicit action on return ("you were away 6 days; move 12 overdue home tasks?"), where a human confirms once and the change is attributable. Location proposes; it never disposes.
  - **The design that defuses it: a mode, not a location rule.** Don't wire geofences to suppression. Make `away` an explicit, server-side, platform-neutral mode that location can *propose*; it stays visible, manually settable and clearable, so a failed geofence degrades to "set it yourself" rather than "broken". It generalises to travel and sick days with no coordinate involved, gives an off switch that isn't a revoked permission, and keeps the web app working. Auto-entry should suggest, not act — for an ADHD tool, an app that silently reshuffles commitments on a guess spends trust faster than it earns it.
  - Also pinned: a home-radius geofence says "not at home" when you leave for milk, so *away* needs distance AND duration thresholds that will be wrong for months; `isNotifiable()` stays the single gate with away as a further suppression rather than a parallel one; the escalation ladder must pause rather than accrue while suppressed; crisis notifications ignore the mode entirely.
- docs(ios): away mode — the cleanup is the problem, not the toggle [XS]
  - Owner reframed the request (2026-07-29): "I already have a vacation rule. The issue is remembering to set it. If I travel for a couple of days before I've remembered, I've created an obligation where I have to go back and fix a bunch of dates because I wasn't engaged to go poke at the app."
  - **Measured what `vacation_mode` actually does, and it is less than assumed: it freezes the streak, full stop.** `server/db.js:1332` is its only consumer (mirrored client-side at `src/store.js:917`). It suppresses no notifications and never touches a due date. `rolloverPlan()` doesn't touch `due_date` either — it un-commits and bumps `boomerang_count`. **Nothing in the system moves a due date automatically.** So auto-setting vacation mode would NOT have prevented the date cleanup; the obvious feature solves a problem the owner does not have.
  - **The valuable half is retroactive repair over a window, not detection.** "You were away Tue–Fri; 12 tasks came due then and are overdue — move them to this week?" One action instead of date surgery, and it works *after the fact*, which is the only help that lands when the whole premise is that you weren't engaged with the app.
  - **`vacation_mode` already carries the window** (`vacation_started` / `vacation_end`) — it just only feeds the streak. Teaching that existing window to suppress while active and offer bulk repair on exit, settable **retroactively**, is the smallest change that solves the stated problem.
  - **Ship the platform-neutral half first:** the repair needs no geofence, no Always permission, no iOS-only code and carries no risk of silent suppression. Location only automates detecting the window — it saves typing two dates, and is the smaller, riskier, more expensive half.
  - Retains the hazards for the detection layer if it is ever added: a false "away" silences your life and silence is unfalsifiable (same shape as `last_sync_error`), so suppression must be stated in the digest; exit and entry are not equally reliable; never auto-change due dates on a location guess; `isNotifiable()` stays the single gate; the escalation ladder pauses rather than accrues; crisis ignores the mode.
  - **Owner redirected the detection half to the CALENDAR, and it is better than geofencing on every axis.** Travel is already on the calendar, and AI can read it: "you're in Wisconsin this week, probably can't do the tasks at home."
    - **It is ahead of time, which geofencing structurally cannot be.** A geofence fires when you are already gone — by then the tasks have come due and the obligation already exists. A calendar event is known days early, so the window is proposed *before departure* and the mess never happens. Arrival detection was always solving it too late.
    - **Stated intent, not inference** — which removes the false-away failure mode entirely. And it works when the phone doesn't: dead battery, denied location, an undelivered region event.
    - **Nearly all existing parts:** `GET /api/gcal/events` lists a time range, OAuth auto-refresh is done, the AI gateway has direct precedent for reading calendar data (`inferEventTime()`), `weather_latitude`/`weather_longitude` give a home reference, and `vacation_mode` carries the window. The geofence path by contrast needed a new table, new native code, an Always permission, the 20-region cap, and was iOS-only.
    - Design: pre-filter structurally (multi-day/all-day, has a location, next N days) before spending a token — "lunch with Ben in Madison" must not trip it; suggest in the morning digest rather than minting a notification; stamp provenance if dates move.
    - **Scope correction:** geolocation is unnecessary for the away-window feature *specifically* — it says nothing about arrival reminders, which still want geofencing and have no alternative, since a calendar cannot tell you that you just got home. Phase 7 is now split into **7a — arrival-triggered reminders (needs geolocation, iOS-only, needs a `places` table)** and **7b — away window (calendar, works everywhere, no places table)**, with a table making the split explicit. They share only a definition of *home*: a region to monitor for 7a, a point to measure "far from" for 7b.
  - Recorded in `wiki/iOS-Native-App.md` under Phase 7. Docs only — no code changed.
- docs(ios): record the requested location-triggered tasks feature [XS]
  - Owner request (2026-07-29): "I have a task I need to do first thing when I get home — when I get home, the app should detect that and notify me." The pattern exists in Reminders and Things.
  - **The 20-region cap shapes the whole design.** iOS monitors at most 20 regions per app, so a geofence per *task* hits the wall immediately and then needs a prioritisation scheme nobody can reason about ("why didn't it remind me?" becomes unanswerable). Geofence **places** instead — a small `places` table that tasks reference — and you monitor ~5 regions regardless of task count, asking "what's waiting here?" on entry. Matches how the request is actually phrased ("when I get home", not "at this coordinate"), and moving house becomes one edit instead of forty.
  - **Arrival is a routine, not an event.** You get home most days, often repeatedly; naive fire-on-entry means the same nudge every evening until the task is done — exactly the fatigue the Great Alert Deletion ended. Needs once-per-arrival-per-day at minimum, and probably "only when the task is live".
  - **A genuine architectural exception, flagged now rather than discovered later:** this is a LOCAL notification fired on-device by iOS, so it never touches `pushNotifications.js` and `notifsMuzzled` does not apply. Quiet hours therefore have to be enforced by the app itself, because the OS won't.
  - Greenfield: **no places concept exists anywhere** — the only coordinates in the system are the single `weather_latitude`/`weather_longitude` pair. New table, new native code, an **Always** location permission (the heaviest the app would ask for), and a server→phone sync of the active region set, since a task completed on the laptop must stop firing on the phone. Nearest relatives are the `inside`/`outside` context tags and `energy: 'errand'` — decide whether places supersede or complement those before building both.
  - Recorded in `wiki/iOS-Native-App.md` as Phase 7. Docs only — no code changed.

- docs(packages): record the requested "tracking drives due dates" feature [XS]
  - Owner request (2026-07-29), from a real miss: "Change fan blade arms" was due yesterday and undoable because the arms were still in transit, so the date got moved by hand. Linking the package to the task would let the date follow the parcel.
  - Recorded in `wiki/Claude-Notes-Integrations.md` under Package Tracking, with the design traps rather than just the ask. **It is the inverse of `signature_task_id`** — that points at a task the package *created* and *completes*; this points at a pre-existing task the package *gates*. Different lifecycle, so it wants its own link table (`task_id`, `package_id`, `role`), which also covers a repair waiting on several parts.
  - **The durability trap is the important part.** Packages self-delete: `auto_cleanup_at` removes delivered rows after `package_retention_days` (default 3). A task whose due date moved because of a package will, within days, join to nothing — so the provenance must be stamped onto the task when the date moves, not derived from a row scheduled to vanish. Straight application of the CLAUDE.md rule: ask what happens to this value when its rows are deleted; the only acceptable answer is "nothing."
  - Other decisions pinned before code: push the date out but **never pull it in** (early arrival isn't evidence you wanted to start sooner); a blocked task must not nag or escalate, so it has to be visible to `isNotifiable()` and the escalation ladder; **no new notification** — the unblock folds into the existing delivery ping or the digest; and an undelivered package must not block a task forever (lost parcels, Amazon TBA ids that never resolve). Hook point is `applyTrackingResult()`, the one place tracking transitions already happen.
  - Docs only — no code changed. One open question left for the owner: ETA Thursday → due Thursday, or the day after?
...
- fix(ios): the share sheet's list fetch failed silently [S]
  ...
  - Moved onto `BoomerangAPI` instead of a hand-rolled `URLSession` call, so it inherits the **10s timeout**. `URLSession`'s 60s default inside a share sheet reads as a hang, not a failure. The hand-rolled trailing-slash trimming was redundant — `BoomerangShared.apiBase` already strips them.
  - **Not a bug, recorded so it isn't chased again:** "add milk to the grocery list" reaching Reminders is correct. Apple requires `\(.applicationName)` in every App Intents phrase, so a phrase without "Boomerang" in it cannot route here. The registered phrasing is "add to \<list\> in Boomerang".
  - **Unbuilt.** No Mac in this environment. Written against the compiler's fragile spots deliberately — the tuple-returning `compactMap` carries an explicit signature, and the main-thread hop uses this file's existing `DispatchQueue.main.async` idiom rather than introducing `MainActor.run` into code that can't be compiled here.
...
- feat(lists): Siri and the share sheet can add to a list [L]
  - Closes the last two list tasks. Both sat on one decision the owner made explicitly (2026-07-28): **ambiguity is asked about, never guessed.** These lists are shared with someone else, so filing groceries into the wrong store's list is worse than one extra Siri turn — and a share that silently lands somewhere you didn't look is worse than one extra tap.
  - **`server/listMatch.js` — the fourth pure module** (after `listMerge`, `listExpand`, `listOrder`): no db, no network, no clock. Tiered matching — exact beats prefix beats substring beats card name — where a tier with one hit resolves and a tier with several **returns the candidates instead of falling through**, because a weaker tier can only widen an already-ambiguous set. Speech normalization strips filler ("the … list", "my") and folds curly apostrophes, so "the trader joe's list" resolves. **Orphaned lists are never a target** — adding to one would write into a checklist that no longer exists on Trello. 17 tests, most of them about the refusals.
  - `GET /api/intents/lists` mirrors `/api/intents/tasks`: says a name, gets back either one match or the candidates. Deliberately does not mutate, so resolving is safe to call speculatively; the actual add goes through the normal `POST /api/lists/:id/items` so it gets the same sync kick and the same Trello guarantees as a typed one.
  - **`default_list_id` setting** (ordinary blob key per the `add-setting` skill — not a secret, no cross-device merge semantics, and `preserveAbsentSettings()` covers the stale-client case). Its control lives in the **Lists modal, not Settings**, because that is where you can see the lists you are choosing between; it appears only once there is more than one list. Writes read-modify-write against current settings rather than a captured snapshot, since the modal can sit open while other surfaces write.
  - iOS: `BoomerangListEntity` + `AddToBoomerangListIntent`, splitting "milk, eggs and bread" into three items client-side so the spoken reply can report the real count. **No offline queue, unlike task capture** — a list item is only useful in the shop, and a silent replay hours later would re-add things the other person already bought, so it says it couldn't send.
  - Share Extension gains an "Add to" row: Task or any live list, **asked every time**, no default and no last-used memory. Lists are fetched per invocation so one added on another device is a valid destination immediately, and orphans are filtered out.
  - Verified live: the endpoint resolves "the grocery list" → Grocery (exact), "2026 groceries" → asks between its three checklists, an orphan is unreachable by exact name, and with `default_list_id` set an empty query resolves while an explicit name still wins. The wipe guard correctly rejected two malformed test writes along the way — worth noting it fired exactly as designed.
  - **The Swift is unbuilt.** No Mac in this environment, so the iOS half is written and reviewed but not compiled or run on device; it needs an Xcode build alongside the existing watch/extension targets. ESLint 0 errors, build ✓, `npm test` 151/151 + smoke, `npm audit` clean.
...
- feat(lists): drag items into order, and push that order to Trello [M]
  - The half deliberately held back from the sorting PR. This is the ONE path in the whole feature where a Boomerang gesture rewrites the order of a checklist someone else reads, so it is the narrowest one available.
  - **Only an explicit drag reaches it.** Name/Recent stay pure view state and still write nothing. **Order is never reconciled by the merge, and `planMerge` is not being taught to** — a background poll that "fixed" the order would be indistinguishable from her reordering it herself, and we would fight her every minute. Position is pulled on create and pushed only on drag.
  - **One drag is ONE write.** `server/listOrder.js` (`planMove`, pure, no db/network/clock — the third such module after `listMerge` and `listExpand`) slots between neighbours rather than renumbering, and returns null for a no-op so an accidental tap costs nothing. When the neighbours leave no representable gap — everything at `0`, the normal state for a list built locally before it ever synced — the whole list renumbers **locally** and still only the dragged item is pushed. Pinned by a test that asserts exactly that.
  - **API shape is `POST /api/lists/:listId/items/:itemId/move` with `{before_id}`** — "put this one before that one", the gesture as it actually happens. Deliberately NOT a "here is the whole new order" endpoint: that shape invites renumbering every row, and every renumbered row is a write to someone else's checklist. A Trello failure returns 502 with the items intact, since the local move stuck and only the push failed; the list records why.
  - **Held writes surface on the list** via `last_sync_error`, gated by `DEV_LIST_SYNC_WRITES` like every other push, and the UI says "Reordered here — not sent to Trello". A reorder that silently did nothing is the exact failure this feature exists to avoid.
  - UI: grip on the unchecked items only (reordering the Got pile is busywork), plus a **"Move to the end" drop target that appears during a drag** — every row target means "before this one", so the last slot would otherwise be unreachable.
  - Tests: `scripts/listOrder.test.mjs` (15, the arithmetic — including that a computed midpoint actually sorts where intended, that no-ops return null, and that degenerate/non-numeric positions renumber rather than collide) + `scripts/listMove.test.mjs` (9, the applier against the real db with Trello stubbed at `fetch`). 134/134 total. Route verified over HTTP end-to-end as well.
...
- feat(lists): link scope — sync a whole Trello card or column (server) [L]
  ...
  - **`server/listExpand.js` is a PURE planner** — no db, no network, no clock — exactly like `listMerge.js`. `trelloListSync.js` fetches, calls it, applies. The split held: **link scope changed zero lines of `listMerge.js`**, the one module where someone else's data can be destroyed and which stays pinned by its own 19 tests.
  - **Expansion never deletes and never writes to Trello.** It creates, renames and tombstones Boomerang rows only — adding a checklist to someone's card is a structural change to their board, not something a background poll should do. The applier is verified to work with writes disabled entirely.
  - Names reuse the item-level 3-way rules: she renamed it → take hers; we renamed it → flagged, never auto-pushed to her card; both → conflict; **null shadow → the OTHER side wins**, because pushing an unproven local name would rewrite her checklist title. A vanished checklist is **orphaned, never dropped**, and a returning one is revived rather than duplicated; orphans leave the sync set so they stop erroring against a dead checklist id every minute. Container-level loss guard (>50%, min 3) mirrors the item one. A source only ever touches lists it created, so linking a card cannot swallow a hand-pinned list.
  - **A real bug caught by the new integration test on its first run:** empty names were treated as authoritative, so a failed column-name lookup *blanked an existing breadcrumb* — the card/column labels would have flickered off on any transient Trello error and back on the next poll. Ids are authoritative; names are not, since that lookup is deliberately best-effort.
  - **The `checklists[0]` bug is fixed as predicted rather than patched.** An unpinned list now adopts a checklist only when the card has exactly one candidate; otherwise it says "that card has N checklists — pick one, or link the whole card."
  - Unlinking a source **keeps every list it made** (`source_id` cleared, items intact) — the items are real data and dropping them because a link was removed is a loss nobody asked for. Verified directly against the db, since only expansion sets `source_id` and the HTTP path could not exercise it.
  - Also: `shadow_name` is stripped from every list API response (`publicList`, the twin of `publicItem`) — sync bookkeeping has no business leaving the server; and re-pointing a list at a different card now clears `shadow_name` too, or the next expansion would compare against a name the new checklist never agreed to.
  - Tests: `scripts/listExpand.test.mjs` (21, pure planner) + `scripts/listExpandApply.test.mjs` (9, applier against the real db with Trello stubbed at `fetch`), both wired into `npm test` — 110/110 total. The applier test needed `flushNow()` before `rmSync` in teardown, the same db-persist race already fixed in `capture.test.mjs`; confirmed non-flaky over 5 consecutive runs.

- feat(lists): sort the lists index by name, recent activity, or drag [M]
  - Manual / Name / Recent control in the lists index, appearing only once there is more than one list. Mode persists per-device in `boom_lists_sort_v1` via `safeSetItem` — deliberately NOT the settings blob, which is last-writer-wins and has eaten data twice; which order you like looking at is not worth that risk.
  - **Scoped to LISTS, not items — the safe half.** `sort_order` is a Boomerang-only column and the sync engine never pushes list order anywhere, so dragging lists writes nothing to Trello and cannot disturb a list someone else relies on. Item-level drag is the risky half (`position` is what Trello orders by) and is deliberately left for its own change, with the `DEV_LIST_SYNC_WRITES` gate and `last_sync_error` surfacing that every write to her data gets.
  - **Sort never writes.** Name and Recent are pure derivations of state already in memory; only a drag mutates anything. The trap avoided: a sort mode that "applied" itself by renumbering the ordering column would turn picking a dropdown into a data mutation — and at item level would push a full reorder of her checklist on every change.
  - **`last_activity_at` is derived in the route, not stored, and this was necessary rather than tidy.** `lists.updated_at` only moves when the list ROW changes (rename, link, sync stamp) — adding an item never touches it, so Recent would have ranked lists by when they were last renamed. Verified against a live server: adding an item left `updated_at` at `…25.262Z` while `last_activity_at` moved to `…26.687Z`, and ticking an item moved it again. Costs no extra query (the route already walks those rows for the counts) and changes no existing semantics. Her edits arriving via sync float a list too, which is the point.
  - Drag is handle-only (`GripVertical`). A whole-row drag would compete with the tap that opens the list — on touch every press becomes a candidate drag and the list stops opening reliably. Reorder is optimistic, PATCHes `sort_order` per row, and reloads from the server on failure.
  - Verified: ESLint 0 errors, build ✓, `npm test` 80/80 + smoke, plus live-server checks of the derivation, the reorder, and that reordering leaves the lists unlinked from Trello.
...
- docs(lists): record the requested ordering + nesting follow-ups [XS]
  ...
  - **Nesting is nearly free, because it is already Trello's own shape** — and Trello nests TWO levels above the items, not one. Measured against the live board rather than assumed: column ("Shopping") → card ("2026 Groceries" / "Costco" / "Trader Joe's") → checklist ("Grocery", "Checklist 2") → items. A Boomerang list stays a Trello CHECKLIST, so the merge is untouched; nesting comes from the card and the column, both real Trello containers that round-trip natively and stay visible on her side.
  - **Corrected my own earlier claim in the same session.** The first draft said "cap depth at one level, because Trello has no nested checklists" — reasoned from checklists alone without looking at the board. Checklists don't nest, but nesting doesn't come from them: card and column are containers, and the family's real structure already uses both. Note now says to look at the board before asserting what Trello can hold.
  - Both sibling patterns are live at *different depths* and the UI must handle each: sibling cards in a column (Costco / Trader Joe's), and sibling checklists in a card (the requested "Groceries has Target and Walmart and HyVee").
  - **Naming collision flagged as a live footgun:** Trello's "list" is the board COLUMN; Boomerang's "list" is a Trello CHECKLIST. Field is to be named `trello_column_id` (Trello's `idList`), never the bare word.
  - **Related bug found while checking:** `syncList` adopts `checklists[0]` when none is pinned. Multi-checklist cards were treated as the exception; the live board shows they are normal, so silently adopting the first of several is now actively wrong — linking must require an explicit choice or create one list per checklist.
  - **Link scope — sync a CARD or a COLUMN, not only a checklist.** The natural consequence of the structure, and it dissolves the `checklists[0]` bug rather than patching it: "which checklist?" stops being a question once the thing you link is the container. `checklist` → one list; `card` → one list per checklist, auto-discovering new ones; `column` → one list per checklist on every card, auto-discovering new cards too.
  - Auto-discovery is the point: today, if she adds a checklist or a new store card, Boomerang never learns of it — "never open Trello again" quietly stops being true and nothing says so.
  - **Architecture: expansion is a SEPARATE concern from merge.** A `list_sources` table holds what was linked; a sync first expands a source into leaf checklists, then runs the existing merge per leaf, unchanged. `listMerge.js` stays pure and stays pinned by its 19 tests; all new complexity lands in the separately-testable expansion step.
  - Three container-level hazards recorded, each the direct analogue of a rule already learned at item level: the >50% wipe guard needs a container-level sibling (0 checklists for a card is a bad response, not a mass delete); creation gets heavier the higher the scope (a column-scope create makes a CARD on her board, visible in her board view — card scope is the safe default); and `lists` needs its own `shadow_name`, since without a 3-way baseline "I renamed the list" is indistinguishable from "she renamed the checklist".
...
- feat(settings): drill-down index replaces the tab strip (rebuild PR2) [M]
  ...
  - **The tabs overflowed on a phone** — "Notifications" clipped to "Notifica" with nothing indicating the strip scrolled — and, being pure chrome, told you nothing. The index rows carry live value summaries instead: `Kept · Dark`, `Due +7d · Stale 7d`, `12 labels`, `Notion · Trello · GCal`, `Push · Pushover` (or `Off`), `53 tasks`.
  - **Amended the design language's own §2.4 summary rule first**, per its closing instruction, and recorded the amendment inline in the doc. The original rule — "if it would need a fetch, show nothing" — read literally leaves **Integrations and Data permanently blank**, i.e. a third of the root screen reduced to chrome, which destroys the single argument that chose an index over tabs (§6: "its rows carry the value summaries"). It was also factually wrong for this deployment: the server reports Notion/Trello/GCal as configured **from env**, so a settings-only check renders empty on the exact machine this is built for. New rule: a summary **may** resolve asynchronously provided it resolves to a value — empty while in flight, never a spinner, never a placeholder, never prose; a *permanently* empty summary means the row should not be a summary row.
  - The Integrations summary reads the **same two sources `IntegrationsPanel` itself reads** (`getKeyStatus()` plus each integration's own status endpoint) and mirrors its `connected` predicates one for one, so the index can never disagree with the page it points at. Renders `None` once resolved with nothing connected — empty means in-flight only. Every fetch failure is swallowed: settings must open whether or not the server answers.
  - Same discipline on the local summaries — the theme parse is copied from the General panel's own, and `Due +Nd`/`Stale Nd` use the panel's `?? 7` fallbacks rather than the store defaults, so the index number always equals the number on the page. Notification channels gate on `=== true`, matching `NotificationsPanel` (`push_notifications_enabled` has no store default and is `undefined` until first opted in).
  - Page identity stays component state and is never persisted — the settings blob is last-writer-wins and UI chrome must not ride it. Both `Settings → Integrations` cross-links from the Tasks panel now drive `setPage`; `IntegrationsPanel`'s own `setActiveTab` prop keeps its name and is passed `setPage`, so that panel is untouched until its own PR.
  - ESLint clean, `npm run build` ✓, `npm test` 80/80 + smoke.
...
- docs(settings): a design language for the settings surface [M]
  ...
  - **The hierarchy is measurably inverted:** the row label renders `600 11px` ALL-CAPS in `--v2-text-meta` while its description below is `12px` sentence-case in the *same colour* — the description is literally larger than the label it belongs to. And `FormDisclosure`'s own doc comment promises "collapsed rows show a summary of what's set inside", but every caller passes prose about what lives inside, never a value. So a collapsed row carries no information and still costs a tap.
  - **Five principles**, each naming the failure it prevents. Headline: **a row at rest shows its value** — a summary is a value (`Kept · Dark`, `7 days`), never a description. Others: the label is the biggest thing in the row; one level of hiding ever; one component per job; descriptions are earned, not default.
  - **Eight row kinds** (toggle, value/picker, segment, navigation, action, destructive, status, free-text/credential), each with anatomy and a 52px whole-row touch target. Trailing lucide chevrons only; the detached `▸` glyph is banned.
  - **Navigation: the six-tab strip is deleted for an index that drills down.** Scrollable tabs rejected (hidden off-screen state; horizontal scroll fights the iOS sheet-dismiss gesture), further tab-merging rejected (the current six ARE already a merge — see the file's own comment at :240-247). Decisive argument: index rows carry live value summaries, so the root screen answers "what's my setup?" rather than being chrome.
  - **Honors a standing user request rather than fighting it:** descriptions move behind a per-row `ⓘ`, matching the 2026-07-17 "I want to click on each for a description — otherwise they should be minimized" already quoted in `InfoHintRow`'s comment. Deleting descriptions outright would have re-earned a complaint the user had already made.
  - Uses existing tokens only — **zero new global tokens**; destructive UI re-tokens to the already-universal `--bm-danger`. Presentation only: no PR in the plan touches `update()`, the settings blob, or any storage path, and collapse state is never persisted (the blob is last-writer-wins and has destroyed data twice).
  - **Migration is 7 independently shippable PRs**, whole-page-at-a-time so no screen is ever half-old: components → nav shell (riskiest, shipped alone so it can be judged in isolation on dev) → General+Tasks → Data → Notifications → Integrations → teardown.
  - **Open objection to settle before PR2:** the spec leaves the Integrations and Data index rows with empty summaries, which undercuts the "root answers what's my setup" argument for a third of the index. Which integration tokens are configured is already synchronous settings state, so Integrations can read `Notion · Trello · GCal` with no fetch. Fix before building the nav shell.
...
- feat(lists): bidirectional Trello checklist sync — schema, merge, API [L]
  ...
  - **3-way merge, in its own dependency-free module** (`server/listMerge.js`). Two people edit this list from two devices and Trello checkItems carry no per-item modification time, so a two-way diff cannot tell "I changed this" from "she changed this" and collapses into last-writer-wins — silently eating one person's edit every time both touch the list between polls. The `shadow_*` columns hold what the two sides last AGREED on; each side is compared to that baseline separately. Fields resolve independently, so her renaming an item while you check it off is cooperation, not a conflict. On a genuine collision Boomerang wins and says so (no timestamp exists to pick the newer honestly, and losing your own just-made edit is the more confusing failure).
  - **Safety posture, because this is someone else's data.** Nothing is ever deleted on Trello as a side effect of a merge — a remote delete happens only from an explicit Boomerang delete, which arrives as a tombstone. Soft deletes exist because a hard delete is indistinguishable from an item Trello hasn't sent yet, so the next poll would faithfully resurrect it. A poll that has lost >50% of the previously-synced items (min 3) is treated as a bad response rather than a mass deletion — the `/api/data` wipe guard's reasoning — with deletions skipped for that round and the reason recorded in `last_sync_error`, while unrelated merges in the same round still go through.
  - **A null shadow falls back to the LOCAL value as baseline**, so an unproven difference reads as a remote change and gets pulled. Caught by a test that expected the opposite: adopting the remote as baseline makes every difference look like a local edit and pushes unproven local state over hers — the one direction that can destroy someone else's data. When we can't prove who moved, the other side wins.
  - **Polling, not webhooks** — the server is tailnet-private and can't receive callbacks, the same constraint already recorded in `shippoTracking.js`. `startListSyncPolling(60s)`, plus a fire-and-forget kick after every local mutation so a change reaches the other person in seconds. A **dev-shaped server merges inbound but never writes back** (`DEV_LIST_SYNC_WRITES=1` to opt in): a staging box fighting production over a real family list would corrupt someone else's data while looking like a sync bug.
  - **19 unit tests** (`scripts/lists.test.mjs`, wired into `npm test`) covering both simple directions, same-change convergence, genuine collisions, independent-field cooperation, deletion in both directions, the never-resurrect rule, and four wipe-guard cases including "an empty remote checklist never wipes a populated list". The merge is pure precisely so all of this is testable with no database, network or Trello account.
  - Endpoints: full CRUD for lists and items (items accept `names[]` so a whole shopping run lands in one call — the right shape for voice), `POST /api/lists/:id/sync`, and `GET /api/trello/lists/:id/cards`, the missing rung of the discovery ladder (boards and lists existed, cards did not) so a card can be picked without opening Trello. Re-pointing a list at a different card clears every held remote id, which would otherwise address check items on the old card.

## 2026-07-26

- docs(watch): record the unresolved phone↔watch connection failure [XS]
  - Work on the watch paused here, so the state is written down rather than carried in anyone's head. Both flavors install, launch and render on the wrist — the install saga is genuinely closed — but every WatchConnectivity request still fails with `WCError.deliveryFailed` ("Payload could not be delivered"), and the `30600ce` activation fix did not resolve it.
  - `wiki/iOS-Native-App.md` gains an **⛔ OPEN** section recording: what is verified correct and must not be re-investigated (shared protocol file compiled into both targets, `WatchBridge` in the App target's Sources, reply handler always fires, companion ids correct, and that `isReachable` is checked first so this is not a range/pairing failure); the one unanswered question that splits the problem in half (does Refresh work with the phone app in the **foreground**?); and three unrun checks in dependency order, with the note that rebooting to clear wedged WatchConnectivity state must come last because it destroys the evidence.
  - Cross-linked from the tech-debt ledger in `wiki/Claude-Notes-Platform.md` so it is reachable from the platform notes, not only from the iOS page.
  - Also renamed the stale heading "The watch app must be signed *for watchOS*" to "The watch must be in the signing profile" — the old title asserted the very claim retracted in `d45cac9`, which is exactly how a wrong idea survives a correction.
  - Method note recorded alongside it: two confident theories this session ("the watch isn't a registered development device", "the profile must list watchOS in `Platform`") were argued from plausible mechanisms, never tested, and both wrong — the second was encoded as a doctor check that then misdirected weeks of work. Measure before asserting; when one cheap observation splits the hypothesis space, take that observation first.

- fix(watch): activate WCSession on background launch, not scene connect [S]
  - First run of the watch app on real hardware: it launched, rendered, and every request failed with **"Payload could not be delivered."** Not an install or signing problem — that saga ended when the watch app appeared on the wrist — but a plain runtime bug that had been unreachable until then.
  - `WatchBridge.shared.activate()` was called only from `SceneDelegate.scene(_:willConnectTo:)`. When the watch calls `sendMessage`, iOS launches the phone app **in the background** to answer it, and a background launch connects no UI scene — so that method never ran, `WCSession` never activated, the message had nowhere to land, and it came back as `WCError.deliveryFailed`. The session was inactive for precisely the launches that existed to serve the watch.
  - Moved to `AppDelegate.didFinishLaunchingWithOptions`, which runs on every launch including background ones. `activate()` is now idempotent (overlapping launch paths), and `sessionDidDeactivate` clears the flag before re-activating so watch switching still re-registers the delegate.
  - Diagnostic worth keeping: `WatchStore.send()` guards on `session.isReachable` before sending, so a genuine range/pairing failure says "Phone not reachable". **Reachable-but-undeliverable means the phone was there and not listening** — that pair of messages distinguishes the two cleanly. The other tell is that everything works while the phone app happens to be open in the foreground, which is the one condition under which the phone-proxy design is pointless.

- fix(watch): the doctor's watchOS platform check was wrong — check devices [S]
  - **Retraction.** `watch-icon-doctor.sh` failed any build whose embedded profile's `Platform` array lacked `watchOS`, on the theory that watchOS would refuse a profile issued for another platform. That theory was invented, never verified against an actual install, and is **false**: a watch app embedded in an iOS companion is provisioned as part of the iOS app family, so `["iOS","xrOS","visionOS"]` is correct for it. The check reported `platform WRONG` on every build for weeks and was the single biggest source of misdirection in this whole investigation — several "fixes" were aimed at satisfying an assertion that never should have existed.
  - Proven by installing the bundle the doctor was condemning: `xcrun devicectl device install app --device 00008310-… …/App.app/Watch/BoomerangWatch.app` → `App installed: bundleID ryakel.boomerang.app.dev.watchkitapp`. Same `Platform` array, same bundle, no rebuild. (First attempt returned `Failed to allocate RSD device` — a transient tunnel error; the retry succeeded.)
  - **What actually gates the install is device coverage.** watchOS refuses a development-signed app whose profile does not list that watch. Before registration the profile was the team wildcard with 1 device (the phone); after, an explicit `iOS Team Provisioning Profile: ryakel.boomerang.app.dev.watchkitapp` with 2 devices — and it installed. The doctor now extracts `ProvisionedDevices` and checks the watch's UDID is in it, printing `Platform` as informational only. When no watch is visible it reports coverage as unverifiable rather than failing.
  - `ios-deploy.sh` and `watch-register.sh` export `BOOMERANG_WATCH_UDID` so the doctor reuses the already-resolved watch instead of repeating the live devicectl query. This also un-blocks the direct watch install in `ios-deploy.sh`, which is gated on the doctor passing — the bogus platform failure is why the watch app was never pushed to the wrist even when the bundle was fine.
  - Lesson recorded in the file header and in `CLAUDE.md`: only assert what has been observed to matter. Every other check in this doctor (icon name, `Assets.car` contents, companion id) was grounded in a real failure; this one was grounded in a guess, and it cost more than all the real bugs combined.

- fix(watch): name the real blocker — device registration, not Developer Mode [XS]
  - With `find-watch.sh` finally returning a usable UDID, the register branch ran for the first time and xcodebuild produced the message that had been unreachable behind an empty string all along: `error: Device "Ryan's Apple Watch" isn't registered in your developer account. The device must be registered in order to be included in a provisioning profile.` followed by `Provisioning profile "iOS Team Provisioning Profile: *" doesn't include the currently selected device … (identifier 00008310-001605683C40E01E)`.
  - **`-allowProvisioningUpdates` does not register unknown devices.** It renews profiles and mints certificates, and it fails outright on an unregistered device. The whole premise of `watch-register.sh` — that using the watch as a build destination registers it — is therefore only half true: the build destination is what makes Xcode *request* a watchOS profile, but the UDID must already be filed under the **watchOS** device class in the portal, which is separate from iOS. The wildcard profile reporting `1 devices` (the phone) was the tell.
  - `watch-register.sh` now tees the build log and recognises this failure, printing the portal URL, the required platform, and the exact UDID to paste, instead of dumping raw xcodebuild errors. It also recognises a generic build failure rather than continuing on to the doctor as if the build had worked — the previous version ran `xcodebuild` bare under `set -e`, so a failure aborted the script with no explanation at all. Not checking the pipeline's exit status under the tee (that is tee's status, and `PIPESTATUS` is not POSIX sh); xcodebuild's own `BUILD SUCCEEDED` banner is the signal. Grep deliberately omits the apostrophe — xcodebuild mixes straight and typographic ones in the same sentence.
  - `watch-icon-doctor.sh`'s `platform WRONG` remediation now names portal registration too, since a build destination alone cannot get past it.

- fix(watch): take the watch UDID from the live query, not the listing [XS]
  - Follow-up to the stale-cache fix, caught in its first real run. The register branch fired for the first time — `Watch found (30B4B8C9-…) — registering it and rebuilding once.` — but that is devicectl's `identifier` GUID, not the hardware UDID the previous commit promised. `list devices` does **not** populate `hardwareProperties.udid` for a watch at all (it does for the iPhone, which is what made the assumption easy to make and easy to miss), so the `hw.get('udid') or d.get('identifier')` fallback silently took the GUID branch. `watch-register.sh` then hands that to `xcodebuild -destination "id=…"`, which matches only the UDID.
  - `find-watch.sh` now uses `identifier` purely as the query key for `devicectl device info details` — it is the id the listing reliably has — and reads the UDID out of the live response (`• UDID: 00008310-001605683C40E01E`), which is the only place it appears. Extraction verified against the real output shape. A candidate that gives no live answer still returns its identifier, since without a response there is no UDID to have.
  - `watch-icon-doctor.sh`'s remediation text replaced. It had said "Enable Developer Mode on the Watch, then pair it for development in Xcode (Window > Devices and Simulators)" — advice that was printed for weeks while Developer Mode was already on, and which sent the diagnosis down the wrong road repeatedly. It now names the actual mechanism: the wildcard persists until the watch is used as a **build destination**, which is what registers it with the account, and `ios:prod` / `ios:dev` already do that themselves.

- fix(watch): find-watch trusts a live Developer Mode read, not the listing [S]
  - **Root cause of the whole watch-install saga, finally measured.** `xcrun devicectl list devices` serves a *cached* record, and its `deviceProperties.developerModeStatus` had gone stale: it reported `disabled` for a watch that `xcrun devicectl device info details` reported live as `Developer Mode Status: Enabled (1)`. `find-watch.sh` filtered on the cached field, so it returned nothing — and every downstream consequence followed silently from that one empty string.
  - The chain, end to end: empty `find-watch.sh` → `ios-deploy.sh`'s register-and-rebuild branch is guarded by `[ -n "$WATCH_ID" ]` and never runs → `watch-register.sh` never builds the watch scheme against the watch → the watch is never used as a build destination, so it is never registered with the developer account → automatic signing cannot issue a watchOS profile → it falls back to `"iOS Team Provisioning Profile: *"` (`Platform = [iOS, xrOS, visionOS]`) → the watch app builds, signs, embeds and passes `ValidateEmbeddedBinary`, and watchOS refuses to install it. Every prior fix in this section was downstream of a device-selection filter reading a stale cache.
  - **Second bug in the same file, which would have bitten immediately after the first was fixed:** it printed devicectl's `identifier` (a GUID, `30B4B8C9-…`). devicectl accepts that, but `watch-register.sh` feeds the value to `xcodebuild -destination "id=…"`, which matches only the hardware UDID (`00008310-001605683C40E01E`). The file's own comment asserted "Watch UDIDs are plain UUIDs, so the hardware-UDID regex the iPhone finder uses does not transfer" — measurably false; a real Apple Watch Ultra 3 reports the same UDID shape as the iPhone. Now prints `hardwareProperties.udid`.
  - New selection rules: enumerate physical watchOS devices from the listing (simulators still rejected on `reality` — registering one accomplishes nothing while looking like it worked), prefer a connected tunnel, then confirm Developer Mode with a **live** `devicectl device info details` per candidate. Reject only on an explicit live `Disabled`, which is trustworthy and is the one case where the caller's "enable Developer Mode" instructions are correct. A watch that gives no live answer at all — asleep, off the network, query failed — is returned as a fallback rather than dropped: the caller then fails with a real error from xcodebuild or devicectl, which beats the silent empty return that caused this bug.
  - Ruled out by direct measurement first, so none of it needs revisiting: icon artwork (all 34 PNGs square, correctly sized, no alpha), asset catalog structure (correct `watch` idioms both flavors), `Assets.car` contents (`AppIcon` / `AppIcon-Dev` compiled in, 1.29 MB / 1.48 MB), `CFBundleIconName`, the watch target's build settings and `TargetAttributes` (all four configs `ProvisioningStyle = Automatic`, `SDKROOT = watchos`, correct companion ids), the `Watch` / `Watch Dev` schemes, and compile-time platform (genuinely `WatchOS27.0.sdk`, `arm64-apple-watchos10.0`). Creating explicit App IDs for `…watchkitapp` / `…dev.watchkitapp` changed nothing and was not the problem.

- feat(watch): ios:prod / ios:dev own the watch end to end [S]
  - Per user: the watch should ride along with the normal builds, not need its own command. `ios-deploy.sh` now does the whole watch side itself — build, verify, register if needed, rebuild, install on the wrist — and grew from 5 steps to 6.
  - **Self-healing registration:** when the doctor fails and a physical watch is visible, the script registers the watch (builds the watch scheme against it), rebuilds once so `App.app/Watch` carries the re-signed copy, and re-checks. Harmless when registration was not the problem: it only builds a target that was already building. On failure it prints the last 25 lines of the registration log and keeps the full log path rather than re-running the whole thing to show output.
  - **Direct watch install** after the phone install, best-effort — installing the phone app only makes the watch app *offerable* in the Watch app's list, it does not put it on the wrist. Never fatal; a watch-side refusal is not a build problem and the phone app is already on by then.
  - `scripts/find-watch.sh` extracted so the device-selection rules (physical only, via devicectl's `reality` field) live in one place instead of drifting between the deploy and register scripts. `ios:watch-register` stays as a standalone escape hatch.
  - Caught two real bugs in review before shipping: a `[ … ] && rm …` that would have taken the whole script down under `set -e` right before the install step, and a failure path that re-ran the entire registration a second time just to display its output. Control flow verified by dry-running all four branches (healthy / register-succeeds / register-fails / no-watch) against stubbed helpers — every branch reaches the install step, none exits non-zero.

- feat(watch): watch schemes + one-shot device registration [S]
  - The signing-platform diagnosis was right but not actionable: `xcrun devicectl list devices` showed the Apple Watch as `available (paired)`, so Developer Mode was already on — yet the profile stayed the iOS wildcard with **1 provisioned device** (the phone). Seeing a device is not registering it. Xcode registers a device with the developer account when the device is used as a **build destination**, and that is what causes automatic signing to issue a watchOS profile.
  - **The gap was mine:** the project had only `App` and `App Dev` schemes, both iOS. Nothing in it could target the watch as a destination, so the watch could never register and automatic signing was stuck on the wildcard permanently. Added shared `Watch` / `Watch Dev` schemes (buildable `BoomerangWatch.app`, Debug/Release and Debug-Dev/Release-Dev).
  - **New `npm run ios:watch-register [config]`** (`scripts/watch-register.sh`): finds the paired physical watch with Developer Mode on via devicectl JSON (rejecting simulators), builds the matching watch scheme against it with `-allowProvisioningUpdates`, then re-runs the doctor to confirm the profile now covers watchOS. Needed because Xcode 26 moved the Devices and Simulators window — this path doesn't depend on finding it. One run, then the normal one-liners work.
  - Also fixed `ios-deploy.sh`'s failure summary, which hardcoded "has no usable icon" and so contradicted the doctor output printed directly above it once the platform check started failing.

- fix(watch): doctor checks the signing platform, not just the icon [S]
  - The icon fix landed and was **confirmed by measurement** — `AppIcon` / `AppIcon-Dev` compiled into a 1.29 MB / 1.48 MB `Assets.car`, correct `CFBundleIconName`, in both the standalone watch product and the copy embedded at `App.app/Watch/`. The watch app still would not install, which is what finally separated the two symptoms that had been treated as one bug all along.
  - **Actual blocker:** automatic signing gave `BoomerangWatch.app` the iOS wildcard profile — `"iOS Team Provisioning Profile: *"`, `Platform = [iOS, xrOS, visionOS]`, **no watchOS**. The phone app got its own real profile, so the build succeeded, `ValidateEmbeddedBinary` passed, the phone installed, and only the watch refused with a bare "App could not be installed at this time". Xcode falls back to that wildcard whenever it cannot issue a watchOS profile, which is the case until the paired Watch is a registered development device — a Mac-side fix (Developer Mode on the watch, then pair for development in Xcode), not a repo change.
  - `watch-icon-doctor.sh` now decodes `embedded.mobileprovision` from each bundle and reports the profile's name, platform list and device count, failing with the remediation steps when watchOS is absent. A watch app with no embedded profile at all fails too.
  - Also corrected in `wiki/iOS-Native-App.md`: the earlier "no `CompileAssetCatalog` step, therefore a stale asset catalog" reading was wrong — Xcode 26 renamed the task to **`LinkAssetCatalog`** (`note: Emplaced …/Assets.car`). The catalog had been compiling correctly the whole time. The product-deletion step added alongside it is harmless and still guards genuine staleness, but it was not the defect.

- fix(watch): watch icon plist key + a doctor that reads the bundle [S]
  - Third icon fix, and the first one grounded in a measurement rather than a theory. Verified first that the artwork is not at fault — both 1024s render correctly (dark plate, orange boomerang; Dev adds its banner) — and that the pbxproj wiring is correct: `Assets.xcassets` is in the watch target's Resources phase, the group path resolves to `ios/App/BoomerangWatch`, and `ASSETCATALOG_COMPILER_APPICON_NAME` is set per flavor.
  - **Real defect found:** the watch target sets `GENERATE_INFOPLIST_FILE = NO` and ships a hand-written `Info.plist` with no `CFBundleIconName`. Asset-catalog icons are resolved *by name* at runtime, so there was nothing to resolve. Added `CFBundleIconName = $(ASSETCATALOG_COMPILER_APPICON_NAME)`, which picks `AppIcon` / `AppIcon-Dev` from the one plist. This had been wrongly ruled out on 2026-07-26 with "the phone app has no such key either" — the phone target gets it merged in from actool's generated plist, which only helps a target whose catalog actually recompiled, and the watch's evidently didn't (no `CompileAssetCatalog` line for `BoomerangWatch` anywhere in the build log).
  - **Forced invalidation:** `ios-deploy.sh` now deletes the previous `*-watchos/BoomerangWatch.app` and the embedded `App.app/Watch` before building. Xcode tracks the catalog by its outputs and edits *inside* an `.appiconset` have repeatedly not invalidated it; missing outputs force actool to run. Cheap, and it removes the whole "reused stale output" class rather than asking anyone to remember `rm -rf ios/build`.
  - **New `npm run ios:watch-doctor`** (`scripts/watch-icon-doctor.sh`): reports bundle id, companion id, `CFBundleIconName`, `Assets.car` size, and whether the named icon is compiled in (`assetutil --info`), for both the standalone watch product and the copy embedded in `App.app/Watch/` — those can differ. `ios-deploy.sh` runs it after each build and warns loudly, non-fatally, when the watch app has no usable icon; a watch app without one does not just look wrong, watchOS declines to install it. The lesson is recorded as an invariant: three fixes were spent because an empty icon set, a stale catalog, and a missing plist key all end in BUILD SUCCEEDED with an identical symptom.
  - Also noted for later: watchOS masks icons to a circle. The prod glyph clears the inscribed circle; the Dev banner's ends do not and are clipped. Left alone deliberately so this change has exactly one variable.
...
- feat(watch): Apple Watch app — Today view proxied through the phone [L]
  ...
  - Pieces: `Shared/WatchProtocol.swift` — the message contract, ONE file compiled into both targets so the sides can't drift; `App/WatchBridge.swift` — phone-side `WCSessionDelegate` (activated in `SceneDelegate`, which also pushes a snapshot as application context on foreground so a wrist-raise shows real content immediately); `BoomerangWatch/WatchStore.swift` + `TodayView.swift`. Watch caches the last payload in its own `UserDefaults` (titles only) and labels it "showing last synced" until a fresh fetch lands. Server refusals (e.g. the three-task ceiling) pass through verbatim; unreachable phone says so plainly instead of spinning.
  - **pbxproj:** hand-wired new watchOS target — 4 build configurations (prod/dev × Debug/Release, `…watchkitapp` / `…dev.watchkitapp` bundle ids, `WKCompanionAppBundleIdentifier` via `BOOMERANG_COMPANION_ID`), Embed Watch Content copy phase, target dependency. Validated by ID-graph check (103 definitions, zero dangling refs, zero orphans, balanced braces) since no Xcode here; `buildImplicitDependencies` is already on in both schemes so `npm run ios:prod` / `ios:dev` pick it up with no script changes.
  - Verified: full suite 61/61 + build + smoke (no server or web changes in this step); Swift + project structure compile at the user's next rebuild — a first interactive ⌘R may be needed to register the new bundle id with automatic signing.

- feat(ios): App Intents expansion — dynamic task entity + complete/commit/snooze/today [L]
  - Step 5 of the sequence (task model → digest → auth A/B → BoomerangKit → **App Intents** → watch). Implements the core of the queued `wiki/UPCOMING_FEATURES.md` "Siri / App Intents expansion" spec, upgraded with the task-model verbs that now exist.
  - **`BoomerangTaskEntity`** — the first dynamic AppEntity: Siri resolves "which task?" against the new `GET /api/intents/tasks` (pure `intentTaskRows()` in `server/taskModel.js`: title substring / exact ids / suggestions; actionable states only, ranked committed → boomeranged → open → shelved; done/archived never match; cap 12). Entity resolution means task titles can now appear IN spoken phrases ("Mark ⟨task⟩ done in Boomerang").
  - **Four new intents** in `BoomerangIntents.swift`, all through BoomerangKit's new `BoomerangAPI` helper (device token first, legacy fallback, 10s timeout, native-never-refreshes invariant): Complete (idempotent, relays refusals), Commit (three-task-ceiling 409 read aloud as-is), Snooze (optional date, default tomorrow 05:00 local — lands before rollover/digest), Today (read-only `/api/today` summary — committed three with first steps, gently-returned count, pool size; CarPlay/HomePod-friendly). Verb intents deliberately do NOT reuse the capture offline queue — acting on stale task state later is worse than asking again.
  - **Verified:** 3 new `intentTaskRows` tests (ranking, ids resolution, suggestion cap — suite 61/61 + smoke); live drill of the endpoint (search/suggestions/ids against a scratch server). Swift compiles at the next Mac rebuild (both schemes) — that rebuild is required for the new intents to appear in Shortcuts/Siri.
  - Remaining from the queued spec (still in `UPCOMING_FEATURES.md`): What-now pick, habit log, escalation-attempt log.

- feat(auth): Phase B — real App Attest verification against the pinned Apple root [L]
  - The on-device Run check (v2.39.0 native build) returned `server_pending`, proving the native flow end-to-end on real hardware — the exact unblock the honest-501 posture was waiting for. Same-day server side, closing the last stub in the auth sequence.
  - **`server/appAttest.js`** (dependency-free): minimal CBOR decoder (RFC 8949 subset) → cert chain to the **pinned** Apple App Attestation Root CA (`server/appleAppAttestRootCA.pem`, fetched from apple.com/certificateauthority, valid 2045) → nonce == SHA256(authData ‖ SHA256(challenge)) vs credCert's `1.2.840.113635.100.8.2` extension (small DER walker; byte-pattern OID search is safe post-signature-verification) → keyId == SHA256(uncompressed P-256 point) == credentialId → rpIdHash == SHA256(App ID), counter 0, aaguid `appattest`/`appattestdevelop`. Never returns ok on a partial check.
  - **`POST /api/auth/device/attest`** now verifies for real: consumes the single-use challenge, verifies, and binds `{key_id, public_key, counter, environment, attested_at}` to the device row (`recordAttestation` — public key + counter reserved for future assertion checks, where counter regression = cloned key). Registry rows gain `attested_at`/`attest_environment`; secret material still never listed. Allowed App IDs default to the two team-scoped bundle ids, override `APP_ATTEST_APP_IDS`.
  - **Failure is loud:** a bad attestation from an authenticated caller fires the `attest_failure` security alert (`reportAttestFailure` — same allowed-loud category as refresh reuse; dev-muzzled). Expired/reused challenges are an ordinary 400 retry, not an alert.
  - **Verified:** 7 new tests (`scripts/appattest.test.mjs`) build a synthetic openssl chain (test root via the verifier's `rootPem` override, leaf carrying the real nonce OID) — valid chain passes, tampered nonce/App ID/keyId/chain/counter all rejected, garbage never throws; full suite 58/58 + smoke. Live drill: forged attestation → 400 + `attest REJECTED` log + alert path, challenge single-use enforced, missing fields 400. The real-Apple-root path is confirmed by the device's next Run check (expected: `verified`).
  - Gotcha recorded: node 22 `test(..., {skip: null})` runs the test but REPORTS it skipped — always `skip: reason ?? false` (cost an hour of phantom "skipped" coverage).
- docs(wiki): index the 20260725 feature recs in UPCOMING_FEATURES [XS]
  - New "Feature recs (researched 2026-07-25)" section at the top of `wiki/UPCOMING_FEATURES.md`: all ten recommendations as one-liners with a pointer to `wiki/Feature-Recs-20260725.md` for detail, plus cross-references where recs supersede existing roadmap lines (widgets under Mobile Native; pattern recognition under Advanced AI).

- docs(wiki): add Feature-Recs-20260725 — ten researched feature recommendations [S]
  - New `wiki/Feature-Recs-20260725.md`: two batches of five feature recommendations from a full-codebase research pass (task/data layer, Quokka, notifications/integrations, client + backlog ledgers). Theme: lighting up dark-but-shipped infrastructure — the migration-046 pick-three layer, `location_json`, `timer`, `/api/today`, discarded GCal event data, unsurfaced analytics.
  - Batch 1: Pick Three commit ritual UI, calendar-aware capacity, Quokka vision capture, Gmail-aware escalation ladders, geofenced errand recall. Batch 2: ten-minute focus timer, WidgetKit widgets over `/api/today`, unified Suggestions Inbox, stuck-task radar, Flight Log week wrap.
  - Appendix records incidental bugs found while mapping (pattern-review muzzle/timezone gap, `out_for_delivery` toggle gap, client/server scoring divergence, untracked AI search, Quokka wiki drift).
...
- feat(auth): Phase A — per-device rotating token pairs with theft detection [L]
  - Step 3 of the task-model → digest-reshape → **auth A/B** → watch sequence. No spec existed in the repo, so one was written first (`wiki/Auth-Device-Tokens.md` — Phase A design + the full Phase B App Attest spec) and then built.
  - **The problem:** every machine credential was ONE static `API_TOKEN` — no per-device revocation (rotating it logs out everything), no theft detection, nothing for the watch/App-Intents steps to build on.
  - **Phase A (built):** `server/deviceAuth.js` mints per-device pairs — access token `bda_<id>.<secret>` (1h TTL, presented exactly like the legacy token incl. `?api_token=` for SSE; O(1) verification via the embedded id) + single-use rotating refresh token. Registry in the `auth_devices` app_data carve-out; secrets stored SHA-256-hashed only; all compares timing-safe. **Reuse detection:** a superseded refresh token presented again is the stolen-token signature → the device is revoked on the spot and a security alert fires (`sendSecurityAlertPush`/`Email` — the one loud category the digest reshape reserved; deliberately not silenceable by per-type toggles). Endpoints: enroll (gated; bootstrap = legacy token), refresh (open + self-authenticating + 20/min rate limit), devices/revoke/delete. The legacy `API_TOKEN` keeps working unchanged as the bootstrap/fallback; gate-off deployments see zero change.
  - **Client:** `src/apiConfig.js` interceptor now reads credentials PER CALL (the old one captured the token once at install — a rotating token would never have been picked up), proactively refreshes within 5 min of expiry, and does a single-flight refresh + one retry on 401 (falling back to the legacy token if the pair is revoked). `ConnectionSetup` auto-enrolls the device on save; Settings → Data → **Devices & security** lists devices with revoke/remove and a this-device marker.
  - **Phase B (spec + honest scaffolding):** `/api/auth/device/challenge` issues real single-use challenges so the native App Attest side can be developed against this server today; `/api/auth/device/attest` returns 501 — attestation verification without a device to produce one valid test vector would be unverifiable security code. Lands with the shared-Swift-package Mac session.
  - **Verified:** 8 unit tests (`scripts/deviceauth.test.mjs`, wired into `npm test`) — rotation, reuse-revocation + alert hook, expiry, hashed-at-rest, registry hygiene; live drill with the gate on — enroll via legacy token, access passes the gate + SSE, refresh rotates (old access dies), superseded-refresh replay → 401 + device revoked + everything it held dead + alert logged, rate limit 429s, revoke/delete/challenge/501 all exercised, gate-off unchanged.
...
- feat(notifications): digest reshape — one calm morning push + The Great Alert Deletion [XL]
  - Step 2 of the task-model → digest-reshape → watch-app sequence, per spec + two locked scope decisions: (1) the ambient flood is DELETED but the deliberate per-task opt-ins survive as the "intentionally rare, high-value pings" (Critical tag with Emergency paging, escalation-ladder nudges, and the per-task Remind-me toggle — now a single gentle line per opted-in task per day); (2) event pings (package delivered/exception/signature, Quokka plan-ready) stay as opt-in same-day sends while weather folds into the digest.
  - **Deleted, not disabled** (all three engines + client): high-priority 3-stage escalation, generic due-status alerts, stale, nudges (incl. AI nudge generation — `notifAi.js` removed), size-based, pile-up warnings (+ the exemption-label picker), habit behind-pace pokes, weekly suggestion pings, weather alerts (nice-day/bad-weekend/nice-window), email batch mode, the counts-style digest, adaptive throttling (`getEffectiveThrottleMultiplier` + `throttle_decisions` endpoints + the Analytics 👍/👎 review chips), and the entire legacy client-side `useNotifications.js` engine (dead-gated since the Settings IA audit).
  - **The digest is THE notification now** (`digestBuilder.js` full rewrite, consumes the task model): push = "Today's three" + the three titles (~150 chars); expanded = commitment sentences from `intention_when`/`first_step` ("After you pour coffee — file the expense report (start: open the receipts folder)"), a fewer-than-three invite line with the pool count (never auto-commits), a rotating ten-minutes nudge, the aggregate gentle-return line (never itemized in the push; listed without counts in the payload), shelve-snoozes landing today, Monday-only pool health with triage invite, then the retained fold-ins (coming up, recap + rally, growth line, weather). Until the pick-three UI ships, an empty commit list falls back to top due-today tasks (crisis first) so the digest keeps its morning-brief value.
  - **Delivery pipeline** (server.js): rollover → assemble → send as ONE sequence, so the digest always reflects post-rollover state. Minutely scheduler in the user's timezone; once per local day; server-down recovery sends on boot if still before local noon, past noon the day is skipped. Collapse everywhere a re-send could stack: web push notification `tag` (the SW now passes it through — it silently didn't before), APNs `apns-collapse-id`, both keyed `daily-digest`. `GET /api/digest/today` serves the assembled payload (cached per day + data-version — any task write invalidates); `POST /api/digest/test` re-triggers the whole pipeline (replace, never stack). Config-shaped channel skips mark the day so the tick doesn't reassemble every minute; transient send failures leave it open to retry until noon.
  - **Settings simplification:** the per-type matrix shrank to the survivors (escalation, packages, plan-ready); High-priority escalation card, Pile-up card, Weather notifications card, Batch mode, and Digest style all removed; the Morning digest card gains honest scheduling copy; push digest now defaults ON (it's the product's one scheduled notification); crisis rides the channel masters (the highpri toggle it used to ride is gone).
  - **Verified live:** commitment phrasing + invite + ten-minute nudge + returning-today render exactly per spec; rollover-coupling confirmed (a task committed "yesterday" appears as a gentle return, not in today's three); chaos drill — complete, un-complete, commit two more with force, re-trigger ×5 → deterministic content, no errors, force never marks the day; past-noon skip and before-noon boot recovery both exercised via timezone flips; dev-muzzle correctly suppresses the scheduled path; word-boundary banned-words grep clean across the digest builder and all engine notification copy (crisis body rewritten to "due Nd ago").
...
- feat(packages): Shippo becomes the USPS tracking backend — live USPS status is back [M]
  ...
  - **`applyTrackingResult()` extracted in `server.js`**: the status-rank downgrade guard, ETA, signature-task creation/completion, delivered auto-cleanup stamps, and all status-change notifications (delivered/exception/out_for_delivery/signature ×3 channels) moved out of the 17track poll loop into one shared function both legs call — the Shippo path gets identical notification behavior for free instead of a duplicated block.
  - **Wiring:** `SHIPPO_CARRIERS` set (usps) — 17track NEVER touches these numbers; when a Shippo token is configured (`shippo_api_token` setting or `SHIPPO_API_TOKEN` env, `getShippoToken()`), USPS polls on a Shippo leg in the background loop, the add-package route (bounded 8s, same race as 17track), per-package refresh, and refresh-all (which now works Shippo-only when no 17track key is set). `isUntrackable()` is now conditional: USPS is a link-out card ONLY when no Shippo token exists.
  - **Client:** the link-only gate keys off `usps && !shippo_api_token && no events` (the events check covers env-var-only servers); link-only copy now points at Settings → Integrations → Shippo. New Shippo row + token field in the Integrations panel (live token required; non-Shippo shipments bill ~5¢/number).
...
- refactor(ui): Kept More/sidebar consolidation — 9 rows down to 4, Notebook merges Notes + Growth areas [M]
  ...
  - **Mobile More (9 → 4):** Notebook · Arcs · Packages · Settings. Removed as duplicates: What now? (lives on Today), Analytics (header avatar). Folded: Caught → Analytics, Activity log → Settings → Data (the "Open activity log" button already existed there), Growth areas → Notebook.
  - **Desktop sidebar Review group (9 → 5):** Notifications · Notebook · Arcs · Analytics · Packages (desktop keeps an Analytics row — no avatar entry there). Loop suggestions row removed too — the Sparkles button on the Loops surface has been its home since 2026-06-11, same as mobile.
  ...
  - **Caught → Analytics:** new hand-off row on the Overview tab ("Caught — everything you finished") that closes Analytics and opens the Done list. Deliberately OUTSIDE the history-loaded gate so the Done list stays reachable while analytics is loading or empty — Analytics is now its only Kept entry point.
  - Legacy standard-theme surfaces (Header, SystemMenu) deliberately untouched.
  - Verified by headless render (mobile + desktop): More shows exactly the 4 rows; Notebook opens with both composers; sidebar shows the pruned Review group; the Caught hand-off opens the Done modal. (First verify run caught the hand-off hidden behind the empty-history gate — moved out and re-verified.)
...
- feat(ai): OpenAI provider integration + AI usage dashboard [XL]
  - **Multi-provider utility AI.** The utility surfaces (size/energy/impact inference, Polish, reframe, What Now, toasts, nudges, Gmail classification, pattern/tag scans, AI search, growth areas, research notes) now route through two tiers — **Workhorse** and **Quick** — each switchable between Anthropic and OpenAI models in Settings → Tasks → AI models (curated catalog + `provider:model-id` custom escape hatch; defaults stay Sonnet 5 / Haiku 4.5). New `server/aiGateway.js` is the single door: resolves the tier, calls the provider (OpenAI chat completions with `max_completion_tokens` + `reasoning_effort: 'low'` on gpt-5-family — reasoning tokens spend from the output budget, the OpenAI twin of the Sonnet 5 thinking trap), returns `{text, provider, model}`. Client utility calls moved from the raw Anthropic proxy to `POST /api/ai/complete`. Deliberately pinned to Anthropic: Quokka's tool-use loop, research_task/ladder tools, and the vision surfaces (OCR, research attachments). Feature gates now use `aiConfigured(tier)` so an OpenAI-only install doesn't silently lose features. OpenAI key: Settings → Integrations → OpenAI (test = free models-list probe; also in `check_integrations` + `/api/keys/status`); env `OPENAI_API_KEY` supported; key added to the Quokka secret blocklist.
  - **AI usage dashboard (local tracking).** Migration 043 `ai_usage` table — every AI call logged at the gateway/proxy choke points with provider, model, feature tag, tokens, and cost estimated from the new `MODEL_CATALOG` pricing table (longest-prefix matching because providers echo dated model ids like `gpt-5-mini-2025-08-07`). `GET /api/ai/usage?days=N` aggregates totals / by-provider / by-model / by-feature / by-day; rendered as a new **AI tab in Analytics** (est. cost, calls, tokens cards + breakdown tables, follows the range picker). Pinned-Anthropic paths log too: Quokka (`quokka`), research/ladder tools, vision calls (`ocr`/`research` via a `_feature` tag on `/api/messages`).
  - **Live-verified end to end:** scratch server + real OpenAI key — tier override routed a live `pong` through gpt-5-mini, usage logged with correct ~$0.00003 cost estimate, all breakdowns populated; no-key and missing-prompt error paths clean; migration 043 runs; lint/build/tests green.

- fix(ui): attachments are finally openable — in-app viewer with Share/Download [S]
  - Prod report: "uploaded an image and I cannot do anything with that image, at least on mobile." The edit-modal attachment list rendered name + size + a remove button — no open/view affordance existed anywhere, on any platform, since attachments shipped. New `AttachmentViewer` (`src/components/AttachmentViewer.{jsx,css}`): tapping the attachment name (now underlined) opens a full-screen overlay — images render inline (data: URL), PDFs in an iframe (blob URL), text files as a preview; every type gets **Download** plus a **Share** button when the platform supports file sharing (`navigator.share` with a File — on iOS that's the native sheet with Save Image / Save to Files, which sidesteps the unreliable new-tab blob URLs in the capacitor:// shell). All local — nothing fetched, Escape/backdrop closes, sits above ModalShell (z 115) below toasts. on thinking-enabled + tokenizer-tight calls [S]
  - Audit follow-up to the adaptive-thinking incident below ("any other surfaces?"): swept the codebase for the rest of the Sonnet 5 breaking-change classes. Clean: no `top_p`/`top_k`/forced `tool_choice` anywhere, no assistant-prefill request shapes (the adviser always ends requests with a user turn). Real residual risk: on the calls that deliberately KEEP adaptive thinking, thinking spends from the same `max_tokens` pot — the Quokka chat loop's 2048 cap (calibrated when Sonnet 4.6 ran thinking-off) could truncate mid-`tool_use` on a thinking-heavy turn, and `research_task` (2048) / ladder drafting (1200) likewise. Plus Sonnet 5's tokenizer runs ~30% hotter, squeezing the thinking-disabled JSON budgets (Gmail 10-email batch at 2048, `callClaude` at 1024, attachment OCR at 4096).
  - Raised caps (hard ceilings — zero cost unless output actually grows): adviser chat + research_task 2048→8192, ladder draft 1200→4096, Gmail classify 2048→4096, `callClaude` + `researchTask` 1024→2048, `extractAttachmentText` 4096→8192. One-liner budgets (10/60/100/400/700/800) left alone — outputs inherently tiny.

- fix(api): Sonnet 5 adaptive-thinking response shape crashed/degraded every utility AI call [L]
  - Prod screenshot: tapping **Polish** on task notes dumped `undefined is not an object (evaluating '(await Ti(...)).match')` plus the entire prompt into the notes area. Root cause: `claude-sonnet-5` runs adaptive thinking BY DEFAULT when the `thinking` param is omitted — the response's first content block can be a `thinking` block (empty text under the default display), so `data.content[0].text` is `undefined`. Client sites crashed (`callClaude` → Polish/reframe/What-Now/toast/dedup/size-inference, `researchTask`, `extractAttachmentText`); `?.`-chained server sites silently returned `''` and degraded without a trace (Gmail classification, growth-area rephrasing, AI email nudges, weekly pattern/tag scans, Haiku AI search, push-message rewrites). Intermittent because adaptive thinking only triggers when the model judges the task warrants it.
  - Second, fully-silent kill: Sonnet 5 REJECTS non-default sampling params — the Gmail classifier still pinned `temperature: 0`, so every classification call 400'd and email scanning was dead.
  - Fix: `claudeText(data)` + `NO_THINKING` exported from `aiModels.js` (imported by both bundles). `claudeText` collects all text blocks (never index `content[0]`); all 13 utility call sites now use it, and the cheap ones (inference/classification/one-liners) spread `...NO_THINKING` into the body — pre-Sonnet-5 cost/latency, and thinking can't eat a 100-token `max_tokens` from the inside. Gmail's `temperature: 0` removed. Deliberately untouched: the Quokka adviser loop (appends `response.content` wholesale — thinking blocks echo back correctly per the API contract) and `research_task`/ladder-draft (already filter text blocks; reasoning helps there).
...
- fix(routines): server-side duplicate-spawn guard — loops no longer double-spawn across clients [M]
  - Prod report with screenshot: "Take out recycling" ×2 (today) and "Refresh Bot Accessories" ×2 (tomorrow) — one recycling copy auto-tagged `inside`, its twin bare, the fingerprint of two separate clients each running the spawn pass. Root cause: `spawnDueTasks`'s "already has an instance" guard is client-local — two clients opening inside the same sync window (native app + PWA/desktop in the morning) each see no instance, each spawn, and per-record `POST /api/tasks` inserted both blindly. Newly visible now that the native app runs alongside other clients daily.
  - Fix at the only serialization point: `findActiveSpawnTwin(task)` in `server/db.js` + a create-only check in `POST /api/tasks` — a NEW id carrying a `routine_id` is dropped when an ACTIVE twin (same `routine_id` + `due_date` + `title`, status not done/completed/cancelled) exists; response returns the twin + `deduped: true` with no version bump/broadcast. Title in the key keeps stack members safe (same routine+due, different titles); done twins don't block (manual "Spawn now" after completing today's instance stays legit). Guard is on the route, NOT in `upsertTask`, so Quokka's LIFO compensation restores are never silently dropped. Client: `pushChanges` (`useServerSync.js`) sees the flag and schedules a rehydrate so the losing client's phantom copy is replaced by server truth before the auto-sizer touches the dead id.
  - Verified live against a scratch server: race dedupes to the twin, existing-id re-push passes, two stack members insert, done-twin re-spawn inserts, non-routine same-title task inserts. Existing dupes in prod predate the guard — delete one of each pair by hand.
...
- fix(loops): ended loops left on Today forever + "Resting" archive section [M]
  ...
  - **"Resting" section on the Loops tab:** paused loops used to VANISH from the Kept Loops library entirely (`!r.paused` filter — only the RoutinesModal's internal list showed them), and ended loops stayed mixed into the active list. Both now render in a dimmed **Resting** group at the bottom — title, why-chip (`paused` / `ended <date>`), total catches — tappable to the detail page, pencil to the editor. The routine row + `completed_history` are untouched, so this IS the archive: stats survive, nothing spawns or nags.
  - **LoopDetail banner:** a resting loop's detail page says so ("stats are kept, nothing spawns or nags") with an **Edit to resume / Edit to reactivate** button — unpause or clear/extend the end date in the editor and the loop returns to active surfaces with its history intact.
...
- fix(lint): server-wide lint coverage — and it immediately found a real Notion bug [M]
  - The eslint config's server block still targeted the pre-reorg paths (`server.js`, `db.js`) — meaning only TWO server files were ever linted, and post-reorg none were. Now: `server/**/*.js` + `scripts/**/*.{js,mjs}` with `ecmaVersion: 'latest'` (top-level await), and `ios/` + `previews/` added to ignores (the Mac's `ios/build/` DerivedData was getting swept by the pre-push hook — Capacitor's bundled `native-bridge.js` fails eslint, reported as a blocked `git push`).
  - **Real bug found by the new coverage (`no-undef`): `textToNotionProperties()` / `simpleMapToNotionProperties()` were called in `notionMCPProxy.js` but defined NOWHERE** — the 2026-05-24 create-pages schema refactor rewrote the file and dropped the definitions while keeping the call sites. Every string-properties `updatePage()` — the `PATCH /api/notion/pages/:id` title sync and knowledgeSync property updates — has thrown a swallowed ReferenceError since. Both helpers restored verbatim from the pre-refactor implementation (`git show dbb7312`).
...
- fix(notion): MCP resilience — synchronous token flush, dead-token retry parking, iOS re-auth guidance [S]
  ...
  - **Root cause hardened:** Notion MCP refresh tokens ROTATE on every refresh, and the rotated token rode the 1-second batched persist — a container restart inside that window strands the DB with an already-invalidated token (the disconnect mechanism; ~10 rapid deploys that evening made it near-certain). `initNotionMCP`'s injected `setData` now calls `flushNow()` synchronously for `notion_mcp_tokens` writes.
  - **Dead-token retries parked:** the 5-minute reconnect retry loop can never heal an `invalid_grant`/401-shaped failure (the refresh token itself is dead) — it just spammed the log forever. `isDeadTokenError()` now stops the retries and sets an actionable `lastError` ("Notion session expired — reconnect from Settings → Integrations → Notion"), which flips the existing `needsReauth` status so Settings shows the Reconnect banner instead of looking vaguely broken.
  - **iOS re-auth hijack documented in-UI:** attempting the OAuth consent from iPhone Safari bounces into the Notion APP via universal links (where the flow dies) whenever the app is installed — reported as "it keeps kicking me from safari to the app." The Notion integration block now shows the working paths when disconnected: a **private tab** (universal links don't fire there) or a one-time desktop connect (the connection is server-side state shared by every client). A native-shell OAuth flow (system browser + `boomerang://` return deep-link + status polling) remains on the build queue.
...
- fix(store): quota-safe localStorage — the native shell's "Boomerang hit a snag" crash [M]
  ...
  - **`safeSetItem()` in `store.js`** — every localStorage write in the app now rides it (store saves, modified stamps, activity/notif logs, mutation queue, external-sync queue, packages cache, Notion page cache, notification throttle stamps; `apiConfig.js` gets a local try/catch since it must stay import-free). On quota: evict the convenience caches (`boom_activity_log_v1`, `boom_notif_log_v1`, `boom_packages_v1` — all rebuildable or server-backed), retry once, and if the payload still doesn't fit, warn once and continue in-memory. The server is the source of truth; a failed cache write now degrades to "stale offline cache," never a dead app.
  - Deliberately NOT done: stripping attachments from the cached tasks blob itself — a stripped cached record pushed back per-record could erase attachments server-side. The durable upgrade path is moving the offline cache to IndexedDB (no meaningful quota); tracked as future work.

- feat(notifications): Phase 4b — full native APNs coverage as the Push channel's second delivery leg [M]
  - Every notification type now delivers natively to the iOS app. Architecture: **not** a 4th engine loop — `sendPush()` in `pushNotifications.js` is the single choke point every push-type notification already funnels through (crisis, escalation, high-pri, overdue, stale, nudge, size, pile-up, habit pace, routine suggestions, digest, packages, Quokka plan-ready, test), so the APNs leg slots in there: native first, web push second. The long-planned "centralized dispatcher when the 4th transport lands" shrank to this — no new loop, no new toggle matrix, no new throttle keys.
  - **Arbitration (the duplicate-killer):** native landing on ≥1 device skips **Apple** web-push endpoints (`push.apple.com` — Safari/Home-Screen PWA), so a phone carrying both the PWA and the native app gets exactly one banner. Desktop Chrome/Firefox always receive. Native sending 0 (unconfigured/no devices/bad key) falls through to full web push — the native leg can only reduce duplication, never drop a notification. Escape hatch: `push_web_alongside_native` (default off), shown in Settings once a device is registered, for a PWA on a *different* Apple device.
  ...
  - **Live-verified against Apple's real sandbox:** booted with a fake-but-valid P-256 key + registered device → dual-leg send ran, APNs HTTP/2 request reached `api.sandbox.push.apple.com` and got the correct `403 InvalidProviderToken` for fake creds, engine fell through to the web leg cleanly and reported an honest failure. With real credentials that 403 is a 200.

- feat(notifications): channel truth — dev muzzle, push-device registry, honest toggles, panel reorg [M]
  ...
  - **Dev-instance notification muzzle.** A dev-shaped server (`APP_VERSION` = `dev`/`dev-*`) now NEVER background-sends: the three engine loops (and with them the digest), package status sends, weather alerts, and the Quokka plan-ready push are all gated by `notifsMuzzled` in `server.js` (weather check duplicated per-file in `weatherSync.js`, same pattern as `isPileupExempt`). The moment dev shares the user's real Pushover/SMTP/push credentials (a settings copy or one test-config session), every nag fires twice — prod + dev — which is exactly the duplicate class reported. Direct test endpoints stay live so channels can still be validated from the dev UI. Opt out with `DEV_NOTIFICATIONS=1`. Exposed as `notifsMuzzled` on `/api/health`; Settings → Notifications shows a banner on muzzled instances.
  - **Web-push subscription registry.** `GET /api/push/subscriptions` + `DELETE /api/push/subscriptions/:id` (`deletePushSubscriptionById` in db.js) — every live subscription listed with push-service origin (Apple/Chrome/Firefox heuristic) + date in Settings → Notifications → Channels, with per-device Remove, rendered regardless of the master toggle. This is the only way a client can see a STALE device's subscription (e.g. the old Home-Screen PWA still receiving web push that looks like "native iOS push" — invisible from the native app, which can't inspect another context's subscription).
  - **Honest toggle display.** Per-type channel toggles (types matrix, package cards, Quokka plan-ready, digest rows, weather rows) used to render checked-but-faded when their channel master was off — visually "on" while nothing would send (screenshot-reported). Display is now master-gated: a toggle reads OFF whenever its channel master is off.
  - **Panel reorganization.** Notifications tab now reads delivery → content → modes → guardrails → diagnostics: Channels (masters + device rows + registry + APNs) → Deep links (public app URL + Pushover link mode, now with a standard section header + collapse) → Notification types → High-priority escalation → Critical mode → Pile-up → Quiet hours → Daily digest → Email deliverability → Weather → Test channels → Notification history (tests + history moved to the end as diagnostics; Critical moved up beside escalation).

- fix(ios): ios-deploy.sh — robust device detection + bundle-id install guard [S]
  - First real run on the Mac exposed both weaknesses. (1) Device detection grepped `devicectl list devices` table output for the literal word "connected" + a hardware-UDID-shaped token — found nothing on the user's machine, so the script exited before ever building (diagnosed via the missing `Debug-Dev-iphoneos` product; the app that DID land on the phone was an Xcode ⌘R with the prod "App" scheme still selected, which looked like "ios:dev built over my prod app"). Now uses `devicectl list devices --json-output` parsed by an embedded python3 heredoc: prefers `tunnelState == 'connected'`, falls back to any paired device (Wi-Fi pairing reports `available`), takes `hardwareProperties.udid` (falls back to `identifier`). Parser logic unit-tested against a synthetic devicectl JSON shape. (2) New install guard: after the build, PlistBuddy reads the product's `CFBundleIdentifier` and the script **refuses to install** when it doesn't match the scheme's expected bundle id — a config/scheme mixup can no longer silently touch the wrong app on the phone; the built id + display name are echoed before install.
...
- feat(ios): the Mac-session bundle — Share Extension + App Intents + side-by-side Dev app [L]
  ...
  - **Dev app (new):** full second build-configuration set — `Debug-Dev`/`Release-Dev` on the project and BOTH targets, and a shared **"App Dev" Xcode scheme** alongside "App". The Dev app is `ryakel.boomerang.app.dev` / display name "Boomerang Dev" / URL scheme `boomerang-dev` / App Group `group.ryakel.boomerang.dev` / a DEV-badged icon (`AppIcon-Dev.appiconset`, generated from the brand SVG with an ember DEV banner) — installable **side by side** with prod, with its own data container and its own Connection config (point it at tasks-dev). All branding flows through three build settings (`BOOMERANG_APP_GROUP`/`BOOMERANG_URL_SCHEME`/`BOOMERANG_DISPLAY_NAME`) substituted into Info.plist and the entitlements, and read by Swift via `Bundle.main` (`BoomerangAppGroup` key) — one codebase, zero `#if` forks. Server side: `buildDeepLink()` emits `boomerang-dev://` when `APP_VERSION` matches the dev pattern (same detection as `isDevEnv`), so the dev server's Pushover links open the Dev app. APNs: the dev server needs `APNS_TOPIC=ryakel.boomerang.app.dev` (env-driven already).
  ...
  - **Phase 3 App Intents (NEW — `BoomerangIntents.swift`):** in-app AppIntents (iOS 16+, no extra target) — "Add Boomerang task" for Siri / Shortcuts / Spotlight / Action button / Back Tap, with an `AppShortcutsProvider` phrase set ("Throw X to Boomerang"). Reads base+token from the App Group, POSTs `/api/intake`, replies "Caught it" — runs in the background, no app launch.
  - **Validated:** mod-pbxproj structural pass (both targets × all four configs resolve, embed phase + dependency correct, project-level configs present); both `.xcscheme` files parse as XML; both Info.plists parse with the substitution vars; lint 0 / tests 17/17 + smoke / `cap config` parses. Real compile is the Mac session (checklist in `wiki/iOS-Native-App.md`).
  - **The user asked for this well before the Pushover deep-link detour; building it directly now.** Native, Boomerang-branded iOS notifications where **tapping the banner itself opens the native app** — the thing neither web push (opens PWA/Safari) nor Pushover (opens the Pushover app) can do.
  - **Server (`apnsNotifications.js`, added to the Dockerfile runtime COPY list):** APNs HTTP/2 sender with **zero new dependencies** (Node `http2` + `crypto`); token-based auth via ES256 JWT (45-min cache; signing verified against a locally-generated P-256 key — 64-byte ieee-p1363 signature, round-trip verified). Config is env-only like SMTP: `APNS_KEY_P8` (literal/`\n`-escaped/base64), `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_TOPIC` (default `ryakel.boomerang.app`), `APNS_ENV` (`sandbox` default — Xcode debug builds — or `production`). Unconfigured = complete no-op. Device tokens live in the `apns_devices` app_data carve-out (token → timestamps; NOT the clobber-able settings blob); stale tokens (410/`BadDeviceToken`/`Unregistered`) pruned on send. Endpoints: `GET /api/apns/status`, `POST /api/apns/register|unregister|test` — all live-tested against a booted server (status/missing-env report, hex token validation, graceful unconfigured test send, unregister).
  - **Client:** `src/nativePush.js` — `enableNativePush()` (permission → APNs register → POST token, 15s timeout with actionable errors) + `wireNativePushTapHandler()` (banner tap → payload `url` → the shared `applyDeepLink()`, wired in `AppV2` beside the `appUrlOpen` listener). Settings → Notifications → Channels gains a native-only "Native iOS notifications (APNs)" block: server status line, **Enable on this device**, **Send test**. `@capacitor/push-notifications@8.1.2` added.
  ...
  - **Scope honesty:** this is the **pipeline** (register + send + tap-routing + test). Full notification-type coverage (high-pri ladders, overdue, digest, packages — the 4th transport loop, or the long-planned centralized dispatcher) is Phase 4b on top.
  - **Turn-on:** (1) developer portal → Keys → + → enable APNs → download `.p8`, note Key ID; (2) enable Push Notifications capability for the App ID (Xcode does this automatically on next build with the entitlement present, paid team); (3) set the four env vars on the server container; (4) `npm run ios`, then in the app: Settings → Notifications → Enable on this device → Send test.
...
- feat(notifications,ios): Pushover deep links open the native iOS app [M]
  ...
  - **Bundle-check fix (necessary, THREE copies):** the JS-bundle syntax check used `new Function(bundle)` (script mode), which rejects the top-level `import`/`export` Vite emits once anything is code-split — and `@capacitor/app` lazy-loads its web impl, forcing a split. The same `new Function` check was duplicated in `scripts/smoke-test.sh` (pre-push) AND inline in both `.github/workflows/build-and-publish-dev.yml` and `build-and-publish.yml` (the `JS FAIL: Unexpected token 'export'` that failed CI on #661). All three replaced with `node --check` on the fetched bundle (module-aware). Same coverage (syntax only), correct for ESM output, future-proofs the remaining native plugins.
  - **Turn-on (Mac):** `git pull` → `npm run build:mobile` (syncs `@capacitor/app` + the URL scheme) → ⌘R; then Settings → Notifications → toggle "Open Pushover links in the iOS app". No entitlement/capability/paid requirement — it's just a URL scheme.
...
- chore(ios): real app icon — replace Capacitor's placeholder [XS]
  - The committed native project shipped the template's generic icon. Rendered `public/icon-512.svg` (the Kept arc-into-catch mark on the dark ground) at 1024×1024, flattened opaque (iOS app icons reject alpha), into `ios/App/App/Assets.xcassets/AppIcon.appiconset/AppIcon-512@2x.png`. Native-only; the PWA icons are untouched. If the phone keeps showing the old icon after a rebuild, delete the app from the Home Screen once — iOS caches icons per install.

- fix(ios): zero the iOS 26+ auto-obscured viewport insets — layout viewport 96pt short [S]
  - **Diagnosed with a real probe** (Web Inspector finally reachable thanks to the forced inspectability): `innerHeight=778` / `visualViewport.height=778` vs `screen.height=874`, while `env(safe-area-inset-top/bottom)` correctly reported `62px`/`34px` — the layout viewport was short by *exactly* the safe-area sum. Rotation didn't fix it. The iOS 26+ SDK auto-populates WKWebView's viewport-obscuring insets (the `obscuredContentInsets` machinery, Safari 26 cycle) from the safe area for a root-view web view, shrinking the layout viewport — and since the app's CSS ALSO pads with `env()` (the PWA mechanism), everything got double-shrunk: bottom tab bar floating ~96pt above the screen bottom with the WebView's white background showing beneath, header pushed down after rotation. Capacitor 8.4.2 has no iOS-26 handling at all (verified against its Swift sources).
  - **Fix:** `ios/App/App/BoomerangViewController.swift` — a `CAPBridgeViewController` subclass (now the storyboard's root VC, `customModuleProvider="target"`) that sets `webView.obscuredContentInsets = .zero` under `#available(iOS 26.0, *)`, re-asserted in `viewDidLayoutSubviews` so rotation/safe-area recomputes can't re-inset it. CSS keeps sole ownership of safe-area spacing, same as the installed PWA.
  - **Mac report (app now fully renders + syncs):** the Kept bottom tab bar was smashed at the screen bottom — labels clipped by the home indicator, plus a dead band below the bar. Cause: the Phase-1 scaffold set `ios.contentInset: 'always'` in `capacitor.config.ts`, which makes the native WKWebView inset the page content — stacking on top of the app's own CSS safe-area handling (`env(safe-area-inset-*)` + `viewport-fit=cover`, the mechanism the installed PWA already uses). The doubled offset pushed the fixed bottom bar half off-screen. Removed the option (default edge-to-edge) so the CSS is the single source of truth, same as the PWA. Requires `npx cap sync ios` to propagate.

- fix(sync): version-mismatch reload disabled in the native shell — infinite boot loop [S]
  - **Mac report (the launch after the scene fix — app now boots, connects, and authenticates):** stuck cycling `SSE: connected → VERSION MISMATCH — client=v2.16.1-15-gf36e16f server=dev-f36e16f → reload`. The stale-client update path (`useServerSync`'s `fireVersionMismatch` → AppV2 unregisters service workers + `location.reload()` after 1s) assumes reloading fetches the server's new bundle. In the Capacitor shell the bundle is baked into the binary: the client version comes from `git describe` on the build Mac while the server's is Docker's `APP_VERSION` (`dev-<sha>`), so they NEVER string-match — same commit included — and a reload can't change anything → infinite loop. Worse, the SSE mismatch path `return`s before `fetchAndHydrate`, so the app never even loaded data between reloads.
  - **Fix:** `VERSION_CHECKS_ENABLED = !isNativeShell()` in `useServerSync.js`, gating both check sites (the SSE `connected` handler and the on-demand `checkVersion()`). Native bundle updates ship via app rebuilds (`npm run build:mobile` + Xcode), not server pushes — the web/PWA update flow is untouched.
  ...
  - **Fix:** `ios/App/App/SceneDelegate.swift` (window via the Main storyboard's `CAPBridgeViewController`; cold- and warm-start deep links / universal links forwarded to `ApplicationDelegateProxy` — under scenes the AppDelegate url handlers are no longer called) + `UIApplicationSceneManifest` in `Info.plist` (single scene, `UISceneStoryboardFile: Main`, replaces `UIMainStoryboardFile`) + the pbxproj references for the new source file.
  - **`ios/` is now committed** (generated via `npx cap add ios` — works on Linux, it's template extraction). Rationale: the scene migration would be silently lost on every regenerate, Phase 2's Share Extension needs the native project versioned anyway, and fixes reach the Mac via `git pull` instead of hand-edits in Xcode. Root `.gitignore` gained the build-output ignores (`ios/App/build`, `ios/App/output`, `ios/DerivedData`, `xcuserdata/`, `ios/capacitor-cordova-ios-plugins`) alongside the existing generated-file ignores (`public`, `capacitor.config.json`, `config.xml`, Pods). Note for the Mac checkout: the previously-untracked local `ios/` collides with the pulled one — `rm -rf ios && git pull && npx cap sync ios`, then redo Signing team + bundle id once (they live in the pbxproj).

- feat(ios): Phase 1.5 in-app Connection screen [M]
  - **Unblocks device setup without Safari:** even with inspectability forced on (below), the Xcode-27-beta Safari still showed "No Inspectable Applications" — so the Web-Inspector localStorage step (the only way to configure the native app) was pulled forward from the roadmap and built. `src/components/ConnectionSetup.{jsx,css}`: full-screen first-run form (server URL + API token, LoginScreen's visual language), **Test & save** verifies `/api/health` (base URL right) and `/api/auth/status` with `x-api-token` (token right) before persisting via `setApiConfig()`, then reloads so the fetch/SSE interceptor re-installs with the new config.
  - **Gating (`src/App.jsx`):** shows ahead of the auth check when `isNativeShell()` (new helper in `src/apiConfig.js`: `location.protocol === 'capacitor:'`) and no `boom_api_base` is configured — the relative `/api` probes would otherwise fail against `capacitor://localhost` and render an empty app. Reopen paths: native-only "Server connection / Change server…" block in Settings → Data; a "Change server or API token…" escape hatch on the login screen (in the native shell password login is a dead end — cross-origin fetches can't carry the session cookie, the token IS the credential); `?connect=1` on the web build for testing. Reopen plumbing is a one-shot sessionStorage flag (`requestConnectionSetup()`/`consumeConnectionSetupRequest()`).
  - Zero web/PWA impact: the screen only auto-shows under `capacitor:`, and the interceptor remains inert with nothing configured.
  - **Verification:** lint 0 errors; `npm test` 17/17 + smoke green; rendered via `?connect=1` in Chromium (dark Kept theme) — card, labels, error styling, and Cancel behave as designed.

- fix(ios): force WebView inspectability on iOS builds [XS]
  - **Mac build session, next blocker:** Safari's Develop menu showed "No Inspectable Applications" for the simulator — the Web-Inspector `localStorage` step (currently the ONLY way to configure `boom_api_base`/`boom_api_token` on a device, until Phase 1.5's in-app Connection screen) was unreachable. iOS 16.4+ WKWebViews are invisible to Safari unless `isInspectable` is set; Capacitor's `ios.webContentsDebuggingEnabled` claims auto-`true` for development builds but that detection didn't hold on the Xcode 27 beta toolchain. Set it explicitly `true` in `capacitor.config.ts` (release builds inspectable too — acceptable for a single-user personal app; revisit if this ever goes to TestFlight/App Store).
...
- fix(tasks): tasks born with a settled size never got impact/tags/energy inferred [S]
  - **Prod report on `dev-c594f50` (after the starvation fixes landed and visibly worked on quick-add tasks):** some new tasks still got no AI treatment at all. Root cause: the background net keys entirely off `size_inferred` — a task created WITH a size (Quokka passes one on most `create_task` calls; a manual size pick in the add modal does the same) is born `size_inferred=true` and never enters the queue, so impact, auto-tags, and missing energy are never inferred for it.
  - **Fix:** `useSizeAutoInfer` gains a secondary fill-only candidate class — `size_inferred` true, `impact` null and not hand-set, `created_at >= '2026-07-14'` (the impact feature's ship date) — that runs the same single inference call but writes ONLY the still-unset fields (impact, merged tags, energy when missing; never touches the settled size). The `created_at` epoch gate preserves the deliberate lazy backfill: historical settled-size tasks stay at the impact-2 baseline instead of triggering an upgrade-day inference storm. `impact_inferred` is stamped even when the model returns no impact so a task can't be re-picked forever.

- fix(tasks): background AI hooks starved by task-list churn — triage/reality-check/inference never ran [S]
  - **Prod report (with screenshots, minutes after the dev deploy):** "I created the task, it tagged it as critical and then nothing else." Build was current (`dev-0ab833d`), settings correct — the task showed no Triage checklist, no Reality-check verdict, no auto tags/impact.
  - **Root cause, two layers in the shared background-hook pattern (`useSizeAutoInfer` → copied into `useCrisisTriage` + `useRealityCheck`):** (1) the effect marks the task `attempted` and arms a throttle timer, but the cleanup — which runs on EVERY `tasks` identity change — cancelled the timer while `attempted` still blocked a re-pick, so any list churn inside the throttle window (autosave, the server's `crisis_since` stamp echo, SSE refetch) permanently starved the task for the session; (2) even when the timer fired, the post-`await` `cancelled` guard dropped the finished result whenever the list changed during the multi-second AI call — which is the NORMAL state right after creating/tagging a task. The sizer had shipped with this latent shape for months, masked by `handleAddTask`'s direct inference path covering the quick-add flow; the two new hooks have no direct path, so the bug surfaced immediately.
  - **Fix (all three hooks):** cleanup releases the id from `attempted` when the call never started (churn now just re-picks the task next effect run); the result write is no longer gated on effect cleanup (`updateTask` by id is safe under churn) and merges against a `latestTasks` ref instead of the stale closure so a tag/checklist the user added mid-call is never clobbered. Failure semantics: once a call actually runs and fails on a REAL API error (no key, parse error), the task stays blocked until next app load — but NETWORK-shaped failures (iOS suspending the PWA mid-call, offline blips) now release the id for a retry on the post-resume refetch (`isNetworkError()` in `src/api.js`: `inferSize`/`generateCrisisTriage`/`generateRealityCheck` rethrow fetch TypeError/AbortError instead of swallowing them).
  - **Same report, second symptom ("can't navigate away from Quokka without it failing"):** backgrounding the PWA kills the SSE reader and freezes the poll timers mid-turn; the detached server-side runner keeps working, but the client had NO resume path — `tryResubscribe` only ran on initial hydration, so returning to the app landed on a dead stream or a "Could not retrieve response" error with the finished answer sitting in the session buffer. `useAdviser` now re-attaches on every `visibilitychange`→visible (session present + no live stream → subscribe-only resubscribe, buffered events replay, stale error state cleared; dead session 404s silently). Also made the unmount cleanup stream-only and explicitly NOT `adviserAbort()` — the old cleanup's `[]`-frozen `sessionId` closure meant it never actually fired, which was accidentally correct (aborting the detached runner on every page refresh would kill running plans); now it's correct on purpose.

- feat(tasks): DIY-or-hire "Reality check" on repair/construction tasks [M]
  - **User request (verbatim):** "I want to also include some AI push to not let my pride get in the way. I am admittedly not handy. I should stop trying to always fix shit myself. Maybe instead the plan should be to push me for construction and repair like tasks to figure out if I should actually be doing it or hiring it out." Decisions (asked one batch, all recommended): auto-run on EVERY repair-shaped task (no button — pride would never tap "question my competence"), the verdict STARTS at hire-out and DIY has to earn it (trivial jobs only; water/gas/electrical/roof/structure = automatic hire), and nags switch framing once a hire verdict lands.
  - **Migration 042:** `diy_assessed`/`diy_verdict`/`diy_reason`/`diy_first_move` on tasks. Detection is a free deterministic regex (`isRepairTaskShape()` in `src/store.js` — strong repair nouns/verbs; bare "fix" only counts with physical/errand energy); the assessment itself is one AI call via the new `useRealityCheck` background hook (mirrors `useSizeAutoInfer`/`useCrisisTriage`, gated by new `diy_reality_check` setting, default on) → `generateRealityCheck()` in `src/api.js` with the hire-default stance baked into the prompt (custom instructions can carve exceptions).
  - **Nag framing on 'hire' (all three engines):** critical bodies swap their "First move" for `diy_first_move` and add "hire it out" to the status bits; high-pri bodies append "you decided to hire this out. First move: …"; the quick-win nudge becomes "Make the call" with the hire first-move; the digest suffixes "· hire it out" on Big-rock/Today/Carrying lines. `generateCrisisTriage()`'s prompt now defaults repair-crisis plans to warranty/quotes/book-the-pro after stop-the-bleeding.
  - **UI:** EditTaskModal "Reality check" banner (verdict pill, blunt reason, first move) with a flip-the-verdict override ("I'm doing it myself anyway" ↔ "Actually, hire it out" — 'diy' returns nag framing to normal, no re-run); 🛠 "hire it out" chip on Kept Today/Anytime/Critical/Tasks rows; Settings → Tasks toggle.
  - **Quokka:** new system-prompt rule 11 (push quotes not DIY steps, never validate non-trivial DIY plans, stage verdict flips when the user decides — Impact renumbered to rule 12); `summarizeTask` exposes `diy_verdict`/`diy_first_move`; `update_task` whitelist accepts the four diy_* fields.
...
- refactor(tasks): rename the crisis tag's user-facing term to "Critical" [S]
  - **User request (same day the feature shipped):** "I'd like to reframe crisis across the board to be Urgent … Or maybe Critical?" — resolved via two questions: term = **Critical** (severity state, pairs with 🚨/Emergency, distinct from both the High priority flag and due-date urgency); tag = **renamed to match** (`critical`, not keeping `prio`).
  - **User-facing copy renamed everywhere:** notification titles (`🚨 CRISIS` → `🚨 CRITICAL` in push + email; body "in crisis Nd" → "critical for Nd" in all three engines), the staleness check-in ("Still a crisis?" → "Still critical?", push + EditTaskModal banner + Today-row chip), the pinned section ("🚨 Now" → "🚨 Critical" on Kept Today + Tasks), EditTaskModal checkbox ("🚨 Crisis mode" → "🚨 Critical"), the Settings card ("Crisis mode" → "Critical mode" with reworded hints), the What Now prompt marker/rule (`[CRISIS]`/`CRISIS RULE` → `[CRITICAL]`/`CRITICAL RULE`), and Quokka's system-prompt rule 10.
  - **Defaults renamed:** `DEFAULT_LABELS` entry `{id:'prio'}` → `{id:'critical'}`; `settings.crisis_label` default `'prio'` → `'critical'`; every code fallback updated. **Internal identifiers deliberately unchanged** (`crisis_since`, `crisis_triage_done`, `crisis_label`/`notif_freq_crisis`/`crisis_stale_days`/`crisis_auto_breakdown`, `isCrisisTask()`, `useCrisisTriage`, log type `crisis`) — same renaming-plumbing-provides-no-value convention as Quokka/adviser.
  - **Migration for the hours the old name was live:** a self-extinguishing reconciler effect in `AppV2.jsx` (loadLabels() never merges defaults into existing installs, so a one-shot shim in the loader couldn't cover task tags): renames a stored `prio` label in place (id + name, dropped instead if a `critical` label already exists), rewrites `prio` → `critical` in any task's tags, and normalizes a stored `crisis_label='prio'`. Idempotent; pure no-op once no `prio` remains; delete after prod data can't contain `prio` (same lifecycle as the theme shims).
...
- feat(tasks): crisis tag ("prio") + impact-based prioritization [XL]
  ...
  - **Crisis tag (migration 040 — `crisis_since`, `crisis_triage_done`):** `isCrisisTask(task, settings)` exported from `db.js` (mirrored client-side in `store.js`); per-task crisis loop in all three engines (`pushNotifications.js`/`emailNotifications.js`/`pushoverNotifications.js`) before the high-pri loop — exempt from its 3-per-tick cap, `notif_freq_crisis` default 2h + avoidance boost, never `adaptiveFreq()`d, bodies carry age/due/"First move: <first open checklist item>". Pushover: priority 1 → priority 2 Emergency at overdue-or-24h-in-crisis with receipt save; removing the tag counts as a resolution (`isResolutionUpdate`) and cancels a ringing alarm; quiet hours use the same per-task `wake-me` gate as high-pri. Crisis tasks excluded from hp/escalation loops + stale/nudge/pile-up pools (crisis outranks an active escalation ladder). `isNotifiable()` treats the tag as an opt-in (undated crises nag; optional `settings` param, fetched once in `filterNotifiableTasks`). `crisis_since` stamped/cleared in `upsertTask` (`applyCrisisTransition`) so every write path is covered. `useCrisisTriage` hook + `generateCrisisTriage()` auto-draft a 3-5-step stop-the-bleeding "Triage" checklist once per crisis (merge-only, flag resets on un-tag). Web-push "Still a crisis?" ping once per `crisis_stale_days` window + EditTaskModal banner (Keep re-stamps `crisis_since`; Demote swaps tag for high_priority). Pinned "🚨 Now" section on Kept TodayView + TasksViewKept (red rows, crisis age, first move, pulsing stale chip); What Now hard rule via new `crisisIds` param (crisis is pick #1, or its first move when the window's too small); Next-up +1000; digest leads Today with 🚨 and includes undated crises. Guardrails: crisis label excluded from AI auto-tag candidates in BOTH `useSizeAutoInfer` and `handleAddTask`; Quokka system-prompt rule 10 (only on explicit user declaration, stage a triage checklist alongside). New settings + `prio` default label; Settings → Notifications "Crisis mode" card (label/cadence/check-in/auto-triage); EditTaskModal "🚨 Crisis mode" checkbox with inline "Also wake me for this".
  - **Impact ranking (migration 041 — `impact` + `impact_inferred` on tasks, `impact` on routines):** rides the existing single `inferSize()` call (new rubric: 3 = people-you're-responsible-to/money/health/unblocks, 2 = own commitments, 1 = self-only; assignee = strong 3 signal); `impact_inferred` mirrors `size_inferred`; NULL scores as the 2 baseline (lazy backfill). Routine impact propagates at all four spawn paths like assignee. New pure `impactRank()` + `CRISIS_RANK` in `src/scoring.js` (base + due ladder 80/60/40/20 + weather-window 50 + event-proximity 0→50 ramp + stale decay −15 cap) with 9 unit tests (`scripts/impact.test.mjs`, wired into `npm test`); ctx built by new `src/impactContext.js` from `computeWeatherWindow()` (new export in `WeatherSection.jsx`: good day now, ≥2 of next 3 bad) + `settings.impact_dates` (`[{id,label,date,lead_days,tag}]`, editor in Settings → Tasks, Quokka-editable). Surfaces: Today/Anytime ordered by impactRank; "Impact" sort mode in Kept Tasks; tap-to-cycle dots chip (`src/kept/ImpactDots.jsx`, `handleCycleImpact` threaded through both Kept shells); EditTaskModal Impact segmented control (deselect = back to auto); What Now line data + prefer-higher-impact rule (size-vs-time HARD RULE unchanged); Next-up `+impact×25`; digest "🎯 Big rock today" line (impact-3, due-picked, sync-safe); "That one mattered." toast prefix on impact-3 completions; Analytics "By impact" breakdown (`byImpact` in `getAnalyticsHistory` + AnalyticsModal section). Quokka: `create_task`/`update_task`/`create_routine`/`update_routine` accept `impact` (stamps `impact_inferred`; null returns to inference), `summarizeTask` exposes `impact` + `crisis_since`, system-prompt rule 11.
  ...
  - **Verification:** `npm test` 17/17 (8 existing + 9 new impact tests) + smoke test green (migrations 040/041 apply, server boots, bundle parses); `vite build` clean; eslint 0 errors. `npm audit`: 3 pre-existing high vulns via `@capacitor/cli`→`tar` (build-time devDependency; fix is a breaking major bump — flagged, not taken).
...
- docs(wiki): reconcile settings-reorg + wiki-rebuild branches with the merged screenshot PR [S]
  - **Why:** PR #639 (screenshots) merged into `dev` while PR #638 (settings reorg) and #640 (wiki rebuild) were still open on an older base — both needed rebasing onto the new `dev` tip, and the user specifically asked to check whether any of the newly-embedded screenshots were now stale given the reorg.
  - **Git:** rebased both branches onto `origin/dev` (0b1c5ca). #638 rebased clean. #640 had one real conflict in `Features.md`'s "Desktop UI" section (both branches touched the same paragraph — kept #639's screenshot embed at the top and #640's "Standard theme:"/"Kept theme — command center" sub-headers below it); `Home.md`/`Kept-Design-Language.md`/`Version-History.md` auto-merged clean.
  ...
  - **`Configuration.md` update:** this file predates both #639 and #640 and wasn't touched by either — its "Settings (in-app)" section still described the old tab layout independent of the screenshots. Reorganized into General / Tasks / Labels / API Keys(Integrations) sub-sections matching the new 6-tab structure, added a Pile-up bullet under Notifications, and noted the Server-logs fold-in under Data.
...
- docs(wiki): audit and rebuild the wiki [L]
  ...
  - **Rewrites:** `Development.md`'s entire Project Structure section described the deleted v1 codebase (`App.jsx`, old `src/components/`, `useSync.js`, `useTrelloSync.js` as a top-level hook) — rewritten to match the current tree (`AppV2.jsx`, `src/kept/`, current hooks, current `npm test`/`npm run build:mobile` scripts). `Features.md`'s Themes/Header Layout/Desktop UI sections described the four-palette Light/Dark/Terminal-Dark/Terminal-Light picker and the pre-Kept "Packages + gear + overflow menu" header — both fully replaced (Standard/Kept families × Light/Dark/System modes; BottomTabs+SystemMenu+SpacesHub for Standard mobile vs. Kept's 4-tab+Throw IA and desktop command center).
...
- refactor(settings): settings IA rethink + dead-setting cleanup [L]
  - **User request:** "take a deep dive through the settings section and help me make it make sense. Shit is everywhere." Follow-on from the pile-up placement fix above — an audit of the full `SettingsModal.jsx` surface found the same split-with-no-cross-reference pattern repeated in several places, plus settings with zero UI control anywhere (dead in practice) and a hand-copied toggle-switch block at ~10 call sites.
  - **Tab structure changed**, not just content moved: `TABS` went from `General, AI, Labels, Integrations, Notifications, Data, Logs` (7) to `General, Tasks, Labels, Integrations, Notifications, Data` (6). The old `AI` tab had exactly one real setting (custom instructions) — folded into a new `Tasks` tab next to the task-behavior thresholds (`default_due_days`, `staleness_days`, `reframe_threshold`) that used to sit orphaned in General with no link to the notification types or AI tone they actually drive. The old standalone `Logs` tab (`ServerLogsPanel`) folded into `Data`, under Activity — same diagnostics category. General is now purely appearance (theme/mode) + Home screen + Build.
  - **Pile-up fully consolidated:** `max_open_tasks` moved out of General into the same Notifications card as `stale_warn_pct`/`stale_warn_days` and the label-exemption picker — one card for everything "too many open tasks."
  - **`public_app_url` relabeled and relocated** from a Pushover-only-looking field inside the Pushover integration block to its own block in Notifications → Channels — it's actually consumed by web push, Pushover, and the daily digest.
  - **Dead settings found and fixed:** `trello_sync_enabled` had no UI control anywhere so the Trello "Sync now" button could never appear (condition now checks `trello_board_id`, mirroring Notion). `src/hooks/usePackageNotifications.js` — a 95-line client-side hook for package delivered/exception/signature browser notifications — was gated behind `notifications_enabled`, which defaults `false` with no UI control anywhere; confirmed dead in practice and deleted outright (server-side push/email/pushover already cover the same events across 3 channels). Closed a real gap while there: added `push_notif_package_signature`/`email_notif_package_signature`/`pushover_notif_package_signature` toggles so signature-required is gated per-channel like delivered/exception already were.
...
- feat(notifications): undated tasks are quiet by default, not just projects [M]
  ...
  - `isNotifiable()` (`db.js`) now requires `due_date || nag_allowed || (escalation_current_rung != null)` for ordinary active-status tasks (previously unconditional true). The escalation carve-out matters: Escalation Ladder tasks are commonly undated by design ("I need a response, no rigid deadline") and are drawn from the same notifiable pool — without this exception, extending the project-style gate to ordinary tasks would have silently broken that entire feature. Verified all branches with a standalone script covering 13 cases (undated/dated × ordinary/project/escalation/gmail-pending/snooze_indefinite/done) before shipping.
  - `EditTaskModal.jsx` gets a "Remind me about this without a due date" toggle for ordinary tasks with no due date (same `nag_allowed` column the Project toggle already used, just no longer gated to `status==='project'`). Quokka's `create_task`/`update_task` `nag_allowed` param now applies to any task, not just projects — previously `create_task` silently forced it to `false` for non-project statuses.
  - Side effect, called out rather than left as a silent surprise: the morning digest's "Carrying" section no longer mentions a purely undated, non-opted-in task (it still carries forward anything with a due date normally) — undated tasks are quiet everywhere now, consistently, not just in real-time push/email/pushover.
...
- fix(notion): create-page ID parsing failed on the hosted MCP server's actual response shape [S]
  ...
  - Fixed: new shared `parseCreatedPage()` helper checks `json.pages[0].id` first (the real shape), falls back to a flat `json.id` for safety, then falls back to the URL regex — which now also matches `app.notion.com/p/<id>`. Verified against the exact response shape from the bug report.
  - `createDatabase()` (a different MCP tool, `notion-create-database`, with no evidence of the same bug) was left untouched — no unverified changes to an unrelated code path.

- fix(ui): Fable-driven design audit — keyboard/scroll bug, Escape gaps, undefined tokens, checkbox/touch-target fixes [L]
  - **User request:** "Use fable to look at all of the design elements of the web page and find bugs... new tasks have a tendency to show up under the keyboard... Also look at edit menus. They look like shit." Four Fable agents audited (1) mobile keyboard/viewport interactions, (2) design-token/color consistency, (3) modal/motion consistency + desktop/mobile parity, (4) edit-modal visual quality. Playwright can't reach even `localhost` in this sandbox (same limitation hit earlier this session), so every finding below is code-verified, not visually confirmed — flagging that explicitly since some of this is CSS-only and worth a real look before promoting to `main`.
  - **The reported bug, root-caused and fixed:** `src/kept/ThrowSheet.jsx` had no keyboard-occlusion handling (unlike `BottomTabs.jsx`/`FloatingCapture.jsx`, which already carry a `visualViewport` listener) and closed without blurring its focused input, so the keyboard's dismiss animation unwound mid-re-render. Separately, nothing in the app ever scrolled a newly-created task into view — combined with `addTask` prepending to state and undated tasks landing in the list's last ("Anytime") section, a new task's actual visibility flipped depending on which date chip was picked, matching the "while inconsistent" description exactly. Fixed: ThrowSheet now has the same `visualViewport` handling as its siblings + blurs before close; `data-task-id` attributes were added to task rows in `TodayView.jsx`/`TasksViewKept.jsx`; `AppV2.jsx` now scrolls a freshly-thrown task's row into view (keyboard-settle delay) via a new `scrollTaskIntoView` helper.
  - **Two more real bugs in the same flow, fixed:** ThrowSheet's "More options" handoff to the full Add modal silently dropped the typed title/date (contradicted its own code comment) — now threaded through `onMoreOptions`/`onOpenFullAdd`. `AddTaskModal` is permanently mounted with no reset, so stale title/notes/tags/size persisted across opens — `AppV2.jsx` now bumps a `key` (`addModalKey`) on every fresh open, forcing a clean remount, and accepts an `initialDraft` prop to seed from the Throw handoff.
  - **Other confirmed bugs fixed:** `ConfirmDialog`'s `autoFocus` sat on the destructive confirm button, contradicting its own "Enter shouldn't auto-confirm" comment — moved to Cancel. `ThrowSheet` and the Kept task-action sheet (`TasksViewKept.jsx`) had no Escape-to-close, unlike every other modal — added. `WeatherSection.css` used legacy `--text`/`--text-dim` tokens that only branch on literal `[data-theme="light"]`, never `"kept-light"` — rendered near-white/dark-gray text in Kept's light theme; swapped for the correctly-branching `--v2-text`/`--v2-text-faint`. `LoginScreen.css` was built entirely against five tokens that don't exist anywhere in the codebase (`--bm-surface`/`--bm-ink`/`--bm-ink-soft`/`--bm-line`) plus a hardcoded Fraunces font-family — always silently fell back to hand-guessed hex; renamed to the real tokens (`--bm-card`/`--bm-text`/`--bm-text-meta`/`--bm-hairline`) and the font to `var(--v2-font-display)`. Two more undefined-token spots (`--v2-border`, `--v2-surface-elev` — used in `EditTaskModal.css` and `SettingsModal.css`, defined nowhere, silently falling back to `currentColor`) pointed at real tokens. An invalid `font: 600 12px/1 inherit` shorthand in `QuickEditTask.css` (CSS-wide keywords can't be a shorthand sub-value, so the whole declaration was dropped) split into individual properties. The checklist-item delete button in `EditTaskModal.css` was `opacity: 0` until `:hover` — invisible and undiscoverable on touch devices, which never fire hover — now visible by default, hover-reveal gated behind `@media (hover: hover) and (pointer: fine)`. `MoreView.jsx` (Kept mobile's "More" tab) never rendered a "What now?" row despite already receiving an `onWhatNow` prop from `KeptShell.jsx` — added, restoring reachability from all four mobile tabs instead of just Today.
...
- feat(weather): surface the "Best days" recommendation + hide badges on weather-independent tasks [S]
  ...
  - Root cause 2: the on-card `WeatherBadge` (both the legacy `TaskCard.jsx` and Kept's `TodayView.jsx`, wired up in the 2026-07-04 weather-badge fix earlier today) only checks `task.due_date` — it was never gated by `resolveWeatherVisibility()`, the same tag/energy-based outdoor-relevance check that already drives the forecast widget and the (also-unused) "Best days" line.
  - Fix: `WeatherSection` now calls `pickBestDays()` on its own forecast data, renders a "☀️ Best days: Tmrw 85°" callout line above the grid, and highlights the picked day(s) with a distinct green outline (separate from the existing amber "due date" highlight — a day can be both). Kept's `TodayView.jsx` now gates its on-card weather badge through `resolveWeatherVisibility({task, labels, weatherEnabled: true}) === 'visible'`, so only outdoor-tagged or auto-detected-outdoor tasks show it — indoor-tagged tasks show nothing, matching the rule the forecast widget already enforced.
  - Known scope limit: the legacy `TaskCard.jsx` (desktop Kanban, Projects view, Stacks) still shows its weather badge ungated — fixing it requires threading a `labels` prop through 4 more parent components that don't currently receive it (`KanbanBoard.jsx`, `ProjectsView.jsx`, `StackSection.jsx`, `ProjectPinnedSection.jsx`), which wasn't part of the reported bug (all screenshots were the Kept mobile Today view). Tracked as a follow-up if it's noticed there too.
...
- feat(theme): add a System option that follows the OS color scheme live [S]
  - **User request:** the theme picker only offered Light/Dark — no way to have the app follow the device's own light/dark setting the way most apps do.
  - `src/theme.js` gains `'system'`/`'kept-system'` sentinel values (one per theme family) stored directly in `settings.theme`. `resolveTheme()` maps a sentinel to its concrete equivalent via `prefers-color-scheme`, and the new `watchSystemTheme(getTheme)` subscribes to OS scheme changes and re-applies the theme live while the app is open — wired into `AppV2.jsx`'s mount effect — so an automatic sunset dark mode flips the app without needing a reload. Settings → General's Mode control is now a three-way Light/Dark/System segmented picker.
...
- fix(kept): wire weather badges into Today view + fix desktop Loop suggestions button [S]
  - **Bug 1 — weather missing from cards.** Outdoor-tagged tasks (e.g. tagged `outside`) showed no weather indicator at all in the Kept Today view, even with a due date inside the forecast window. Root cause: this was a gap, not a regression — when Kept's `TodayView.jsx` was built from scratch (replacing the legacy `TaskCard.jsx`-based list), it never got the weather wiring the old card had. `AppV2.jsx`'s `useWeather()` result was threaded into every legacy v2 list surface but never passed as a prop into `<KeptShell>`/`<KeptDesktop>`, so `TodayView` had no weather data to render regardless.
  - Fix: `weatherByDate={weather.enabled ? weather.byDate : null}` now passed from `AppV2.jsx` into both `KeptShell` and `KeptDesktop`, threaded through to `TodayView`, which computes `weatherDay` from the task's `due_date` (identical lookup to `TaskCard.jsx`) and renders the existing `WeatherBadge` component in the row's meta line for `dayTasks`. Reuses `WeatherBadge`'s existing CSS (`.v2-weather-badge` etc., already shipped in the bundle since `TaskCard.jsx` is imported unconditionally) — no new styles needed.
  - **Bug 2 — suggestions "swallowed."** On Kept **desktop** only, tapping the "Suggestions" button inside the Loops view silently did nothing. Root cause: `KeptDesktop.jsx`'s `<LoopsView>` instantiation never passed `onOpenSuggestions` (introduced in commit `11573ba`, "loop reconcile review surface") — the mobile `KeptShell` passed it correctly, masking the desktop gap. The badge dot (pending count) still rendered since that's a separate fetch, making the button look live while being dead.
  ...
  - Not fixed here (out of scope, flagged for later): the notification bell's `unread` count is also never wired into `KeptHeader` in either shell — the badge dot never lights up even though `NotificationsModal` computes a real count once opened. Separate, small fix, deferred since the user is prioritizing the iOS-native-app push reliability work over notification UI polish right now.

- fix(settings): make Integrations panel rows collapsible again [S]
  - **Prod bug:** every integration row (Anthropic, Notion, Trello, GCal, Gmail, 17track, Weather, Pushover) in Settings → Integrations rendered its full inline config unconditionally — API key fields, Disconnect, Sync Parent, Knowledge Base, all stacked open at once — forcing a very long scroll with no way to fold a row shut, unlike the Notifications tab which still collapses correctly per-section.
  - Root cause: the Notifications panel has a `collapsed_notif_sections` settings-persisted collapse mechanism (`isCollapsed`/`toggleCollapsed` + a `SectionHeader` button with a rotating chevron); `IntegrationsPanel` never had the equivalent — not a regression from a specific commit, this panel's tracked history never included it.
...
- fix(kept): brand-new loops no longer show a false "missed last week" gap [S]
  - **Prod bug:** creating a weekly routine via Quokka (e.g. "Take out trash" every Thursday, "Take out recycling" every Friday) on a day when the scheduled weekday had already passed that calendar week immediately showed a "1 to fix" badge — "caught 0 of last 1 week" — even though the routine didn't exist yet during that window.
  - Root cause: `cycleWindows()` in `src/kept/cycles.js` correctly forward-shifts a weekly routine's anchor to the next occurrence of its `schedule_day_of_week` (mirroring `getNextDueDate`'s fixed grid, per the 2026-06-20 bugfix) — but when that shift pushes the anchor into the future relative to today, `sinceDays` goes negative and the old `Math.max(0, Math.floor(sinceDays / stepDays))` clamp forced the window index back to `0`, minting a window at that future anchor anyway. The window was neither `current` (today hadn't reached it) nor `caught` (no history yet), and `loopGaps()` had no check for "hasn't started yet" — so it fell straight into `missed`. Two independent scheduling functions (`getNextDueDate`, which correctly says "nothing owed yet," and `cycleWindows`, which minted a phantom past window) disagreed.
  - Fix: when `sinceDays`/`monthsSince` is negative (the first cycle hasn't started), `cycleWindows()` now generates zero windows for that routine instead of clamping to index 0. Established routines are unaffected — the clamp only mattered for the negative case, which only arises for a schedule freshly created this way.
  - Added `scripts/cycles.test.mjs` (wired into `npm test`) — regression coverage for the exact scenario, confirms an established routine's genuinely missed past cycles still surface correctly, and confirms a routine created ON its scheduled weekday gets a single `current` (not missed) window. Also fixed `src/kept/cycles.js`'s `../dates` import to the explicit `../dates.js` extension so the module resolves under plain Node's test runner (Vite already tolerated the extensionless form; the new unit test needed the explicit one).
...
- feat(api): one-command live health check across every integration ("check my integrations") [M]
  ...
  - **Notion is split into its two independent paths** so they're tested separately: `notion_mcp` reads `notionMCP.getStatus()` (mcp.notion.com / OAuth), `notion_rest` calls the new `restTokenStatus()` in `notionMCPProxy.js` which validates `NOTION_INTEGRATION_TOKEN` against `api.notion.com` (`GET /v1/users/me`) with **NO MCP fallback** — the one call that confirms a rotated integration token on its own path without the MCP masking a failure. Directly answers "test my new Notion key and make sure it isn't hitting the MCP."
  - Surfaces: `GET /api/integrations/health` (new endpoint) + Quokka read-only tool `check_integrations` (in `adviserToolsMisc.js`, wired through `adviserDeps.probeIntegrations`). Say "check my integrations" / "are my connections working" / "test my Notion key" and Quokka returns the full report. Brings the adviser tool count 63 → 64 (settings+analytics group 4 → 5).
  - New exports: `restTokenStatus()` (`notionMCPProxy.js`), `verifyEmail()` (`emailNotifications.js`). No new files → no Dockerfile COPY change (both modules already in the runtime list). Verified: server boots, `GET /api/integrations/health` returns all 12 with correct statuses on a cred-less DB (11 not_configured, web_push degraded), `check_integrations` present in `GET /api/adviser/tools`.

- chore(repo): stop `Version-History.md` generating merge conflicts (union merge driver) [XS]
  - This changelog is append-near-the-top, so any two parallel or stale-base branches edited the same region → it conflicted on essentially every change (it caused 100% of the day's merge-conflict churn). New `.gitattributes` marks `wiki/Version-History.md` as `merge=union`: git keeps BOTH sides' added lines on merge/rebase instead of emitting a conflict. Scoped to this file only — `union` is correct for an append-only log, wrong for prose files (CLAUDE.md edits replace text, so it's deliberately excluded). Verified by reproducing the exact failing rebase (two branches both inserting at the top): 0 conflict markers, both entries retained. Documented the mechanism + trade-off (entries can interleave in odd order; tidy on sight) in CLAUDE.md's Documentation Requirements.
- fix(routines): auto-roll now re-applies the trigger time when carrying a task forward [S]
  - Prod bug: a daily **auto-roll** loop with a **trigger time** (e.g. "IFR Studying – PM" at 13:00) surfaced in the morning instead of staying parked until its clock time. Cause: `spawnDueTasks` in `useRoutines.js` rolls an auto-roll routine's existing instance forward to today, but the roll path only *cleared* a stale `snoozed_until` and never re-applied `triggerSnooze(today, trigger_time)` — so the rolled-forward task had no snooze and showed immediately. The fresh-spawn path always set it correctly; only the roll path was wrong. Not a regression from the day's UI/vite/changelog work (those don't touch routine logic).
  - Fix: the roll updates now always set `snoozed_until = triggerSnooze(today, routine.trigger_time)` (today@trigger if still future, else null). Added a `needsSnoozeFix` check so an already-surfaced instance (due today, snooze cleared) gets re-parked on the next spawn pass — idempotent (no-op once correct), and a no-op for loops without a trigger time. Verified across 5 scenarios: rolled-from-yesterday → hidden until 13:00; already-surfaced → re-parked; correctly-snoozed → no-op; past trigger (after 13:00) → surfaces; no-trigger loop → unchanged.
...
- feat(routines): loop reconcile review surface — see + fix the days per loop [M]
  ...
  - **Removed the silent auto-reconcile-on-load** (it would have erased the "unrecorded" group before the user could see it). The Quokka `reconcile_loops` tool stays as the explicit fix-all and now also respects `skipped_days`. Threaded `onMarkLoopDay`/`onSkipLoopDay` AppV2 → KeptShell/KeptDesktop → LoopsView → LoopDetail.
