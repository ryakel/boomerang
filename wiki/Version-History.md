...
- feat(ios): Phase 2 — Share Extension ("Add to Boomerang") [M]
  - Native Share Extension target so sharing a text selection / link / web page from any app → **Add to Boomerang** creates a task. `ShareViewController` (`SLComposeServiceViewController`) reads the API base+token from the App Group container (written by Phase 0's bridge), shows an editable compose sheet, and POSTs `{title, notes}` to `/api/intake` with `x-api-token`. No server changes — `/api/intake` already existed for the iOS Shortcut. If the app hasn't been connected yet (empty group), it shows "Open Boomerang and connect first" instead of failing silently.
  - **`ios/` files:** `ShareExtension/{ShareViewController.swift, Info.plist, ShareExtension.entitlements}` + `App/App.entitlements` (App Group on the main app) + a fully hand-authored extension target in `project.pbxproj` (native target, Sources/Frameworks/Resources phases, Embed-App-Extensions copy phase on the App target, target dependency, container proxy, Debug/Release configs, config list). The extension links no Capacitor — raw `URLSession`/`UserDefaults` — so its Frameworks phase is empty.
  - **App Group standardized to `group.ryakel.boomerang`** (aligned to the real bundle id `ryakel.boomerang.app`; Phase 0's placeholder `group.in.kfam.boomerang` updated to match in the Swift literal + docs). Extension bundle id `ryakel.boomerang.app.ShareExtension`, team `L7JZ99D6K5`.
  - **Validated without Xcode:** the pbxproj surgery was scripted with asserted anchors, then loaded + semantically cross-checked with `mod-pbxproj` (both targets resolve, every referenced UUID exists, embed phase `dstSubfolderSpec=13` points at the `.appex`, App depends on the extension, per-config settings present). Reparses clean. Real compile is the Mac loop.
  - **Turn-on (Xcode, one time):** add the **App Groups** capability (`group.ryakel.boomerang`) to BOTH the App and ShareExtension targets in Signing & Capabilities (auto-provisions with the paid team); pick the team on the ShareExtension target if not inherited.

- feat(ios): Phase 0 — native token bridge (App Group) [S]
  - Foundation for every out-of-WebView native surface (Share Extension, App Intents, native push): those run in Swift processes that can't read the WebView's `localStorage`, where the connection config lives. `BoomerangNative.swift` — a `CAPBridgedPlugin` exposing `setSharedConfig`/`getSharedConfig` — mirrors `boom_api_base`/`boom_api_token` into an App Group container (`UserDefaults(suiteName: "group.ryakel.boomerang")`). `src/apiConfig.js` calls it from `setApiConfig()` (every Connection-screen save) and once on interceptor install (covers configs set before this build). WebView stays the source of truth; the bridge only writes what JS hands it.
