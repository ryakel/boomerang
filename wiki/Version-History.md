...
- feat(notifications,ios): Pushover deep links open the native iOS app [M]
  ...
  - **Smoke-test fix (necessary):** the JS-bundle syntax check used `new Function(bundle)` (script mode), which rejects the top-level `import`/`export` Vite emits once anything is code-split — and `@capacitor/app` lazy-loads its web impl, forcing a split. Replaced with writing the bundle to a temp `.mjs` and `node --check` (module-aware). Same coverage (syntax only), correct for ESM output, future-proofs the remaining native plugins.
