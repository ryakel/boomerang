...
- feat(ui): Kept de-generification — Syne display, arc-vocabulary icons, flight-path hero, throw/catch/return motion [L]
  - The "make Kept not look AI-generated" work, landed in the real app (the `kept-preview-p1/p2/p3.html` + `kept-motion.html` prototypes drove the direction; they stay as dev-only render harnesses). Four layers:
  - **Display typeface Fraunces → Syne.** `--v2-font-display` in `src/kept/palette.css` (both kept-dark/kept-light) now resolves to `'Syne'`, matching the `tokens.css` base — Kept stops overriding the display face onto the era's default "warmth serif." The orphaned Fraunces `<link>` was removed from `index.html` (only the dev-preview HTMLs still reference Fraunces).
  - **Custom arc-vocabulary icons** (`src/kept/icons.jsx`, new) replace the stock Lucide nav/loop glyphs so the navigation reads as an authored system: `IconToday` = the Day Arc gauge, `IconLoops` = a return-arc closing into a "caught" dot (also the loop-ring glyph), `IconTasks` = stacked arc-ticks (the section-marker shape), `IconMore` = flight-trail dots bridged by streak arcs. Wired into `KeptNav`, `KeptDesktop` nav, and every `bm-loop-ring` (`TodayView`/`LoopsView`/`LoopDetail`). API mirrors Lucide (`size`/`strokeWidth`) so call sites are drop-in.
  - **Flight-path hero** (`src/kept/DayArc.jsx` rewrite + `viz.css`). The semicircle "fitness gauge" is replaced by a real boomerang flight: a flat out-and-back trajectory (launched from the hand, out over the top, curling the right tip, returning along the bottom). Ember = points flown toward the goal, faint = the remaining return; the boomerang rides the leading edge and spins to rest on entry (rAF, ~1.3s, gated on `prefers-reduced-motion` → static end-state). The count is left-weighted, breaking the centred-gauge symmetry. Same `{value, goal, caption}` contract, so mobile Today + desktop Today rail are unchanged call sites.
  - **Motion vocabulary** (`src/kept/motion.js`, new — spec §8), wired into the real handlers, all WAAPI + reduced-motion-gated:
    - **catch** — tapping a task check plays the gold boomerang dot arcing back into the check as it fills, the title strikes through, the row settles; completion commits ~300ms later so the motion lands on the live row. Wired in `TasksViewKept` + `TodayView` chk buttons (guarded against double-fire).
    - **throw** — a freshly-thrown task's row drops into the list on a soft spring arc (`playThrowIn` after `handleAddTask`, looked up by `data-task-id`).
    - **return** — snoozing flies the row off to the right on an upward arc before it leaves the active list (`returnThenCommit` wraps the SnoozeModal commit; a new "↩ Throw it back — returns tomorrow" row in the Kept action sheet gives mobile a visible, list-triggered return via `onSnoozeBack` threaded AppV2 → KeptShell → TasksViewKept).
  - Rows now carry `data-task-id` for the motion lookups; `.bm-chk`/`.bm-row-title` get `position:relative` + the transient `.bm-catch-dot`/`.bm-catch-strike` styles in `shell.css`. New `src/kept/*` files ship via the Vite bundle (no Dockerfile change). Verified: `npm run build` green, eslint clean (pre-existing warnings only), and a seeded `node server.js` render in kept-dark shows the new type/hero/icons with zero page errors.
...
- docs(ui): Kept "less AI-generated" Plan-1 prototype — Syne display + arc-vocabulary icons [XS]
  - Dev-only render harness `kept-preview-p1.html` (sibling to `kept-preview.html`, never shipped) exploring the highest-leverage moves to de-template the Kept look. Holds the palette constant and changes only two variables so the before/after is isolated: (1) display typeface **Fraunces → Syne** (kills the era's default "warmth serif" tell), and (2) the Lucide nav/loop icons → a **custom arc+dot set** drawn from Kept's own motifs — Today = the Day Arc gauge, Loops = a return-arc closing into a "caught" dot, Tasks = stacked arc-ticks (the section-marker shape), More = flight-trail dots bridged by streak arcs.
  - Verified visually via headless-Chromium screenshots of both prototypes. Findings: the icon swap is the bigger win (the nav now reads as an authored system, not the Lucide starter kit); Syne de-genericizes the type but needs a tracking pass (the negative letter-spacing was tuned for Fraunces) and two minor icon nits (Today rider-dot a hair high, Tasks domes want thickening at 21px). No app code touched — this is exploration ahead of a possible Plan-1 (`--v2-font-display` + `index.html`) implementation and the higher-leverage Plan-2 (structural flight-path hero).
