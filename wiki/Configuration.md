...
- **Growth areas** — standing reminders about yourself ("be patient with the kiddo"), NOT tasks: no due date, no nagging, no score, no completion. One morning and one evening area is picked each day, reworded fresh by AI, and shown as a banner on Today; persistent ones are also injected contextually into What Now and Quokka. Per-area timing (Morning / Evening / Persistent) and day scope (Any / Weekdays / Weekends). Stored in the `growth_areas` app_data collection with dedicated endpoints, NOT in the settings blob. Also editable from the Notebook, and by Quokka ("I want to work on being more patient"). Full spec: `wiki/Growth-Areas.md`.
