...
- **Tag-based visibility override** — tag any task with `outside` to force the weather to show, or `inside` to collapse it into a drawer. The "🌤 Weather" disclosure button is shown for drawer-mode tasks; click to expand. Useful when the auto-detect gets it wrong (e.g. an indoor garden task with `physical` energy).
- **"Hide weather on cards" global toggle** — Settings → Weather → check this to push all card weather UI behind a drawer regardless of tags or auto-detect. Modal still shows weather. The `outside` tag still wins as a per-task override.
