...
- **7-day forecast section on outdoor task cards** — outdoor-leaning tasks (physical or errand energy, or title keywords like "mow", "paint the deck", "wash car") with a due date in the window show a compact 7-day forecast right on the card (no expand needed). Layout is a centered row of 3 days above a smaller centered row of 4 days, each cell showing condition icon, high/low, and wind. The task's due date is highlighted.
- **"Best days" recommendation in the edit modal** — opening one of those outdoor tasks shows a "Best days: …" line above the notes (with a sun icon) listing up to 3 days within the forecast that are best for outdoor work, scored on condition, precip, wind, and temperature comfort. The recommendation reacts to in-modal edits — retag a task to a different energy and the line updates immediately. Rendered live from the cached forecast, never written into the notes field.
