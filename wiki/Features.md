...
- **Auto-roll, habit mode, and historic suggestions** *(planned — spec in `wiki/Activity-Prompts.md`)*: a routine can be flagged `auto_roll` so missed days bump the existing task's due date instead of stacking duplicates (use case: medication you can't double up on). A new `spawn_mode: 'habit'` will support target-frequency tracking ("workout 2x/week") without a cadence-locked schedule — always-visible card with "+ Log it" button, behind-pace nudges mid-week. And a weekly server scan over 12 months of completed-task history will detect recurring patterns and surface them as a new `routine_suggestion` notification type.
