...
- **Closes when you finish its task**: completing a loop's task — from the loop's own check OR from the main task list — crosses out the loop card and ticks the "Loops `done/total`" count on Today. Loops left stuck open from completed tasks (e.g. after a data restore) auto-close on next app open: Boomerang reconciles each loop's history against its finished tasks so it stops nagging and advances to the next cycle.
