...
**Entry point** (`src/App.jsx`): renders `AppV2` (`src/v2/AppV2.jsx`) unconditionally inside the ErrorBoundary, and wires global error logging. The legacy v1 UI (`src/AppV1.jsx` + `src/components/`) was deleted on 2026-06-10; the old `localStorage.ui_version` flag and `?ui=` escape hatch are ignored. `data-ui-version="v2"` is mirrored on the documentElement and `data-ui="v2"` activates the namespaced design tokens (`src/v2/tokens.css`, all `--v2-*`). Shared components formerly in `src/components/` (`Logo`, `Rings`, `CarrierLogo`, `WeatherSection` + `WeatherSection.css`) now live in `src/v2/components/`.
