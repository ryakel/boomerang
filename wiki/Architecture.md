...
**Entry point** (`src/App.jsx`): renders `AppV2` (`src/AppV2.jsx`) unconditionally inside the ErrorBoundary, and wires global error logging. The legacy v1 UI was deleted on 2026-06-10 and the `src/` tree was flattened into `src/` the same day (Kept K0): components in `src/components/`, hooks in `src/hooks/`, Wallaby surfaces in `src/wallaby/`. The old `localStorage.ui_version` flag and `?ui=` escape hatch are ignored. `data-ui-version="v2"` is mirrored on the documentElement and `data-ui="v2"` activates the namespaced design tokens (`src/tokens.css`, all `--v2-*`).

**Theme palette family** (`data-theme` attribute on documentElement): `light` (default), `dark`, `wallaby-dark`, `wallaby-light`. Light + dark token defaults live in `src/tokens.css`; Wallaby palettes in `src/wallaby/palette.css`. The terminal theme was removed 2026-06-10 (Kept K0) — only the migration shims in `loadSettings()` + the index.html pre-paint script survive, upgrading stored terminal values to wallaby. Theme values + browser-chrome colors are registered in `src/theme.js` (`THEME_COLORS`, `applyTheme()`); index.html keeps an inline pre-paint copy.
- `DateField` (`src/components/DateField.jsx`) — bracketed text trigger that opens the native date picker via `input.showPicker()`. Renders `[ due date ]` empty / `[ YYYY-MM-DD ]` filled, plus an inline `× clear` button. Same component shape in all themes; terminal mode flattens the trigger to bare bracketed text via CSS.
...
**Completion stamping — one `completed_history` writer per path (2026-06-08).** The Wallaby grids/streaks read `routine.completed_history`; the habit meter + analytics read done tasks. To avoid double-counting, `completed_history` is stamped exactly once per completion: `completeRoutine` (auto routines, on task completion), `logHabit` (habit-mode born-done tasks stamp inline — previously didn't, leaving habit grids empty), `skipCycle`, and the "Last done" editor. `uncompleteRoutine(id, ymd)` (`useRoutines.js`) is the reopen counterpart — removes one same-day entry — called from `src/AppV2.jsx` `handleUncomplete` (and the Wallaby `onCompleteTask` reopen branch); ordinary routines always, stacks only when un-clearing a fully-cleared cycle. The Wallaby Home checkbox (`onToggleHabit`) is a shortcut into these paths (completes/reopens the real task) — it raw-writes `completed_history` only for **past days** (backfill). Stacks render as per-member rows on Home via `onSpawnStackToday` + `onCompleteTask`.
