...
**Entry point** (`src/App.jsx`): renders `AppV2` (`src/AppV2.jsx`) unconditionally inside the ErrorBoundary, and wires global error logging. The legacy v1 UI was deleted on 2026-06-10 and the `src/` tree was flattened the same day (Kept K0): components in `src/components/`, hooks in `src/hooks/`, Kept's own surfaces (Today/Loops/Tasks/More, the desktop command-center shell, FlightTrail/MonthDots/DensityRibbon/DayArc) in `src/kept/`. The old `localStorage.ui_version` flag and `?ui=` escape hatch are ignored. `data-ui-version="v2"` is mirrored on the documentElement and `data-ui="v2"` activates the namespaced design tokens (`src/tokens.css`, all `--v2-*`).

**Theme palette family** (`data-theme` attribute on documentElement): `light` (default), `dark`, `kept-light`, `kept-dark`, plus `system`/`kept-system` sentinels that resolve live to one of those four via `prefers-color-scheme` (`src/theme.js`'s `resolveTheme()`/`watchSystemTheme()`). Standard light/dark token defaults live in `src/tokens.css`; Kept's `--bm-*` tokens live in `src/kept/palette.css`. Both the Terminal theme (removed 2026-06-10, Kept K0) and the Wallaby theme (removed 2026-06-12, K6) are fully torn out — only the migration shims in `loadSettings()` + the index.html pre-paint script survive, silently upgrading any stored `terminal*`/`wallaby*` value onto `kept-dark`/`kept-light`. Theme values + browser-chrome colors are registered in `src/theme.js` (`THEME_COLORS`, `applyTheme()`); index.html keeps an inline pre-paint copy that mirrors the same sentinel-resolution table (inline scripts can't import modules).
- `DateField` (`src/components/DateField.jsx`) — bracketed text trigger that opens the native date picker via `input.showPicker()`. Renders `[ due date ]` empty / `[ YYYY-MM-DD ]` filled, plus an inline `× clear` button.
- `TypingSuggestions` (`src/components/TypingSuggestions.jsx`) — Quokka empty-state suggestion list where each row types itself in sequentially. Pending/active/complete states render the same element with different visual treatments; completed rows are real clickable buttons.

There are no `check:terminal-*` convention smoke tests anymore — both were dropped with the Terminal teardown. `npm test` today runs `scripts/dates.test.mjs` + `scripts/cycles.test.mjs` (unit tests) followed by `scripts/smoke-test.sh` (build + boot + `/api/health` + bundle-parses check).
...
CREATE TABLE tasks (
  ...
  -- Crisis tag (migration 040):
  crisis_since TEXT,                     -- stamped in upsertTask when the crisis label lands; drives Emergency timing + "Still a crisis?"
...
The `routines` table carries scheduling columns including `cadence`, `custom_days` / `custom_unit` (migration 031), `schedule_day_of_week` (017), `schedule_day_of_month` + `schedule_week_of_month` (034), `spawn_mode` + habit fields (026), `auto_roll` (025), `follow_ups_json` (023), and `trigger_time` (033). `trigger_time` ('HH:MM' 24h, nullable) is the surface-at clock time: on spawn (`useRoutines.js`) the new task's `snoozed_until` is set to that time on its due day so it stays hidden and silent until then. Per-follow-up-step clock times (`at_time` / `at_next_day`) live inside `follow_ups_json` (no schema change) and are applied in `db.js` `spawnNextChainStep`. **Loop reconcile** (migration 037) adds `skipped_days_json` to `routines` — a JSON array of 'YYYY-MM-DD' days the user acknowledged as "didn't do it, move on" from the LoopDetail "Needs attention" surface. `loopGaps()` (`src/kept/cycles.js`) derives the actionable days (unrecorded completions + missed cycles) from the cadence windows; "Mark done" stamps `completed_history`, "Skip" appends to `skipped_days` (excluded from both `loopGaps` and the server `reconcileRoutineHistory`). **Stacks** (migration 035) add `members_json` to `routines` (JSON array of independent member templates) and `stack_bonus` to `tasks`. A routine with non-empty `members` fans out into one independent task per member each cycle (`spawnStackMembers` in `useRoutines.js`); the cycle key is `(routine_id, due_date)`. Clearing the last member of a cycle (detected in `src/AppV2.jsx` `handleComplete` via siblings sharing `(routine_id, due_date)`) stamps `stack_bonus` = 20% of the cycle's combined member points on the closing task — summed into the day's total by `computeDailyStats`/`computeRecords` (`src/scoring.js`) — and advances `completed_history` once. Both new columns flow through `db.js` row mappers + the full task/routine UPSERTs (the partial-update path merges then upserts, so a column must be in the UPSERT to persist).

**Completion stamping — one `completed_history` writer per path (2026-06-08).** Kept's trail/streak visuals (`src/kept/FlightTrail.jsx`, `MonthDots.jsx`) read `routine.completed_history`; the habit meter + analytics read done tasks. To avoid double-counting, `completed_history` is stamped exactly once per completion: `completeRoutine` (auto routines, on task completion), `logHabit` (habit-mode born-done tasks stamp inline — previously didn't, leaving habit grids empty), `skipCycle`, and the "Last done" editor. `uncompleteRoutine(id, ymd)` (`useRoutines.js`) is the reopen counterpart — removes one same-day entry — called from `src/AppV2.jsx` `handleUncomplete` (and the Kept `onCompleteTask` reopen branch, shared by `TodayView`/`KeptShell`/`KeptDesktop`); ordinary routines always, stacks only when un-clearing a fully-cleared cycle. `toggleHabitDay` in `AppV2.jsx` (passed down as `onToggleHabit`) is a shortcut into these paths (completes/reopens the real task) for habit-mode routine cards on Today — it raw-writes `completed_history` only for **past days** (backfill).
...
**Critical loop (2026-07-14; user-facing term "Critical", internal identifiers keep `crisis_*` names).** All three loops open with a per-task critical block (tasks carrying the `settings.crisis_label` tag, default `critical`, matched by the shared `isCrisisTask()` export from `db.js`): fires before the high-priority loop, exempt from its 3-per-tick cap, at `notif_freq_crisis` (default 2h) with avoidance boost, deliberately NOT wrapped in `adaptiveFreq()` (ignoring a crisis must never teach the app to back off). Crisis tasks are excluded from the high-pri/escalation per-task loops and the stale/nudge/pile-up aggregate pools. Pushover escalates crisis sends from priority 1 to priority 2 Emergency when the task is overdue or `crisis_since` is >24h old (receipt → `tasks.pushover_receipt`, standard act-to-cancel path; removing the crisis tag also counts as a resolution). The web-push loop additionally owns the once-per-window "Still a crisis?" check-in (`crisis_stale_days`). `isNotifiable()` treats the crisis tag as an explicit opt-in so undated crises still notify.
...
**Read state vs. engagement (migration 036).** `notification_log` carries TWO independent timestamps. `tapped_at` (migration 020) is engagement analytics — stamped only when a user taps a notification deep-link, keyed by `(task_id, channel)`, and feeds tap/completion-rate metrics. `read_at` (migration 036) is the Notifications-center UI read flag — what "unread" keys off of. They were once conflated, which broke read state three ways: `markAllRead` only mutated client state (never persisted), task-less rows (weather/pile-up/generic) had no `task_id` so could never be marked read, and passive "glance" reads polluted engagement metrics. `db.js` exposes `markNotifEntriesRead(ids)` + `markAllNotifsRead()`; `listNotifLog` returns `read_at`; `POST /api/notifications/log/read` (`{ ids }` or `{ all: true }`) persists it. Because `read_at` rides `notification_log` (which survives bulk wipes — not in the bulk-PUT collection list), read state syncs across devices. The separate `POST /api/notifications/tap` continues to stamp `tapped_at` for real task taps only.
